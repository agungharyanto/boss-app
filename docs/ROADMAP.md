...
| v0.23.0 | Network         | OMCI Integration untuk Zero-Touch ONT Provisioning | **Reservasi slot nomor + nama saja — scope detail BELUM ditentukan.** Terpisah TOTAL dari TR-069/GenieACS (sudah dibangun sejak v0.7.x) — OMCI dipakai ONT yang **tidak** support TR-069. **BOSS App integrasi LANGSUNG ke OLT** (API/CLI native vendor: ZTE, HSGQ, dll) — **TIDAK lewat SmartOLT sebagai perantara/proxy**; tujuannya BOSS App berdiri independen, bukan bergantung ke layanan pihak ketiga. SmartOLT tetap dipakai paralel untuk kebutuhan sekarang (tidak diputus tiba-tiba), tapi arah jangka panjang BOSS App lepas darinya. **Gambaran besar 3 jalur provisioning ONT ke depan**: (1) **TR-069** via GenieACS — sudah ada (v0.7.x); (2) **OMCI** via integrasi native OLT — sprint ini; (3) **DHCP Option 43** — solusi sementara effort rendah, sudah disinggung sebelumnya. Scope detail (vendor OLT prioritas, DHCP Option 43 vs OMCI penuh, mekanisme API/CLI per vendor) belum ditentukan — decision-gate terpisah saat sprint ini benar-benar dimulai (BOSS-003) — jangan asumsikan detail apa pun dari baris ini. Nomor slot `v0.23.0` — `v0.22.0` ("Manajemen User") didaftarkan di branch `manajemen-user` (belum di-merge saat baris ini ditulis). | Backlog |
