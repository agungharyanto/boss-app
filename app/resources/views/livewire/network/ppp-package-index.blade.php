{{--
    v0.14.5 — same conditional wire:poll pattern as hotspot-package-index/
    customer-ip-pool-index/network-profile-group-index, reused verbatim.
--}}
<div class="p-6 max-w-6xl mx-auto" @if ($hasPendingSync) wire:poll.5s="$refresh" @endif>
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-semibold text-gray-800">{{ __('Profil PPP') }}</h1>
        ...
        @if ($canManage)
            ...
            >
                {{ $showCreateForm ? __('Batal') : __('+ Profil PPP Baru') }}
            </button>
        @endif
    </div>
    ...
    @if ($showCreateForm)
        <form wire:submit="createPackage" class="mb-6 p-4 border border-gray-200 rounded-md bg-gray-50 space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div>
                    <label class="block text-sm font-medium text-gray-700">{{ __('Grup Profil (PPP)') }}</label>
            ...
            {{-- Grup Profil (yang menentukan NAS implisit) wajib dipilih
                 dulu sebelum Simpan bisa diklik — pola sama seperti
                 Bandwidth Profile/IP Pool Pelanggan/Grup Profil/Profil
                 Hotspot, defense-in-depth di atas validasi backend
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">{{ __('Masa Aktif') }}</th>
                        ...
                        @else
                            ...
                            <td class="px-4 py-2 text-sm text-right space-x-2 whitespace-nowrap">
                                @if ($canManage)
                                    ...
                                    @endif
                                    <button wire:click="deletePackage({{ $package->id }})" wire:confirm="{{ __('Hapus profil PPP ini?') }}" class="text-red-600 hover:underline">
                                        {{ __('Hapus') }}
                                    </button>
                                @endif
                            </td>
