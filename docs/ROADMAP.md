...
| v0.7.8  | Network         | GenieACS Resolver Multi-Vendor + Auto-WAN Configurable | **(a)** Folding `fix-genieacs-pppoe-provision` (Opsi A) — `CpeParameterResolverService` diperluas (RX/TX/MAC/PPPoE multi-vendor, fallback product_class), `docker/genieacs/presets/default-*.js` (default-pppoe baru + default-optical di-trim ke 4 objek optik fleet), `GENIEACS_MAX_COMMIT_ITERATIONS=128` — semua SUDAH live di GenieACS sejak 2026-09-02, folding menutup drift `main`↔produksi. **(b)** Auto-WAN Configurable — VLAN + username/password default PPPoE yang dulu HARDCODED di provision script sekarang dari halaman "Konfig Remote" (`/remote-config`, `remote_wan_configs` singleton). `App\Services\Network\GenieAcsPresetService` menulis provision `default-wan` + `args` preset `default` lewat REST genieacs-nbi (`PUT /provisions`, `PUT /presets` — dikonfirmasi berfungsi di 1.2.16). Deteksi vendor Huawei/CMCC/ZTE + guard idempoten dipertahankan persis dari script referensi rekan Agung. **(c)** Restrukturisasi sidebar "Perangkat CPE" → grup toggle-murni "Remote" (Perangkat CPE / Cek Status Device / Konfig Remote), pola persis "Komisi"/"Profil Paket". **(d)** Cabang `isCTCom` di `default-wan.js` — data model DOMINAN fleet (~372/415 device), 3 cabang vendor lama tidak mencakup. Deteksi `X_CT-COM_UserInfo.UserName`, VLAN LEVEL-WCD (`X_CT-COM_WANGponLinkConfig.VLANIDMark`), guard idempoten CT-COM TERVERIFIKASI ke device asli (skip). **(e)** Section "Device GenieACS Belum Ter-bind" di `/cpe-devices` + VLAN9-TR069 di ro-hotspot dieksekusi via RouterOS API. **STATUS AUTO-WAN: WAN1 Huawei branch + guard idempoten CT-COM terverifikasi. ~~Provisioning WAN baru ke device CT-COM FRESH = KNOWN BUG~~ (`preset_loop`). **RESOLVED di `v0.7.8.1`** (2026-09-09): fix instance-number (`ctcNewInstance` re-read, tidak hardcode `.1`) + WAN2 dynamic WCD-creation (`ctcResolveWcd` — firmware H3-2S menerima AddObject `WANConnectionDevice`) + LAN binding SSID + remediasi permanen `buildPrecondition([])` (insiden fleet-wide 2026-09-07, 4 ONT). **Auto-WAN CT-COM (WAN1+WAN2+LAN binding+WCD-creation) TERVERIFIKASI PENUH ke `CMDCA473F158`** (H3-2S fw V1.1.20P1T4). Tetap **DISABLED by default** (`RemoteWanConfig.enabled=false`). | Selesai — v0.7.8 merged/tagged; **patch `v0.7.8.1` (2026-09-09) menutup KNOWN BUG + insiden 2026-09-07..09 → Auto-WAN CT-COM VERIFIED PENUH, CLOSED** (merged `develop`→`main`, tagged `v0.7.8.1`; slot ROADMAP) |
...
| v0.9.0  | Billing & Finance | Commission (Approval + Clawback) | Menutup sisa scope cluster Commission. Eligibility (v0.9.5) + Payment (v0.9.11) sudah; sub-versi ini menambah **Approval** (HANYA komisi bulanan recurring/limited_count: Eligible→Approved / Eligible→Rejected+alasan, `CommissionApprovalService`, permission `commission_ledger.approve` — Titip TETAP langsung Eligible setelah OTP, keputusan v0.9.6 tidak dibalik, dikonfirmasi Agung 2026-09-07) dan **Clawback** (semua skema termasuk Titip, semua status termasuk Paid — APPEND-ONLY: baris reversal baru `amount` negatif + `status=Clawback` + `reversal_of_id`, baris asli utuh; clawback baris Paid ditandai `[UTANG]`; permission `commission_ledger.clawback`; keduanya tier-admin). Payout bulanan (`CommissionPayoutService`/`MonthlyPayoutIndex`) sekarang `Approved`→`Paid` (dulu `Eligible`→`Paid`). Migration `reversal_of_id`/`reviewed_by`/`reviewed_at`. `CommissionStatus::Clawback`. **PLUS Bagian B (di luar kode komisi): backup penuh data SmartOLT via API** (langganan berakhir ~2026-09-12) ke `/opt/boss-app-backups/smartolt-export-2026-09-07/` + rekonsiliasi 48 gap ONU → 27 baris `cpe_devices` baru dibuat, 21 sisanya perlu keputusan manual Agung (lihat `RECONCILIATION_2026-09-07.md`). Setelah ini **cluster Commission v0.9.x TUNTAS PENUH.** Bagian B (rekonsiliasi 21 ONU sisa) DITUNDA — 27 baris yang sudah dibuat tetap. | Selesai — merged `develop`→`main`, tagged **`v0.9.13`** (nomor v0.9.13 karena v0.9.1-v0.9.12 sudah dipakai; tag ini menutup slot scope "v0.9.0" Approval+Clawback) |
...
| v0.9.3  | Billing & Finance | Commission Rate Settings      | Tabel `commission_rates` — rate komisi per **Profil PPP** (FK `ppp_packages` saja; Hotspot/Token pakai skema Agent terpisah, di luar v0.9.x). 3 skema: `recurring_amount` (Per Bulan), `limited_count_amount`+`limited_count_times` (skema X-kali, berpasangan, times fleksibel), `titip_amount` (Titip); minimal 1 wajib. REST API + Livewire `/commission-rates` (list SEMUA PppPackage, form rate inline per paket), permission `commission_rates.*` tier-admin. | Selesai — merged ke `main` (commit `d56be0b`, digabung bareng Restrukturisasi Sidebar); kode-nya tercakup di tag `v0.9.4` (tidak dapat tag `v0.9.3` tersendiri) |
| v0.9.4  | Billing & Finance | Skema Komisi per Pelanggan    | Menutup gap v0.9.3: `customers.ppp_package_id` (FK→`ppp_packages`, nullable) — **link komisi SAJA, sepenuhnya independen dari `subscriptions`/billing** (billing masih manual/MixRadius, tidak disentuh). `commission_ledger.scheme` (`recurring`/`limited_count`). Form registrasi: input teks "Paket" → dropdown `PppPackage` aktif + dropdown "Skema Komisi" reaktif (label menyertakan nominal Rupiah: "Per Bulan - Rp 3.000" / "2 Kali - Rp 33.000", via `CommissionRate::schemeOptions()` — satu sumber kebenaran, Titip tidak ditawarkan). `RegistrationService::register()` + `CommissionAttributionService` mengisi `commission_ledger.amount` dari rate; tanpa skema → `amount` NULL (backward compatible). Panel "Paket & Referral" inline di detail pelanggan: `referred_by_referrer_id` null→terisi = buat 1 `CommissionLedger` Pending; referrer yang **sudah** terisi **TERKUNCI** (opsi c — hanya paket yang bisa diubah). **PLUS: pindah link sidebar "Referrer"+"Rate Komisi" dari cluster "Operasional" ke "Billing & Finance"** (Operasional kini 1 item "Reseller" — sengaja dibiarkan, tidak dibubarkan). | Selesai — merged `develop`→`main`, tagged `v0.9.4` |
| v0.9.5  | Billing & Finance | Commission Ledger Auto-Maturity | Hook `InvoiceService::markPaid()` → `CommissionLedgerMaturityService::matureForPaidInvoice()`. `commission_ledger.invoice_id` (FK nullable, `nullOnDelete`) + indeks unik parsial (1 invoice = maks 1 baris komisi). Komisi diperoleh **PER INVOICE LUNAS** (append): invoice pertama matangkan baris "template" v0.9.4 di tempat (Eligible + `invoice_id`, `amount` di-refresh dari `CommissionRate` saat ini); invoice berikutnya = baris baru. `recurring` tanpa batas; `limited_count` di-cap ke `CommissionRate::limited_count_times`; skip pembayaran = tak ada baris (gugur natural). **`subscriptions`/`SubscriptionService`/`GenerateDueInvoices` tidak disentuh.** UI admin komisi lengkap → slot berikutnya (bukan `v0.9.7` — itu dipakai Login Terpadu; UI komisi ke `v0.9.8`+). Sementara ada halaman minimal "Titip Masuk" (`/titip-masuk`, dari v0.9.6). | Selesai — merged `develop`→`main`, tagged `v0.9.5`; verifikasi live 3 skenario dikonfirmasi Agung LOLOS |
| v0.9.6  | Billing & Finance | Fitur Titip (Self-Service + OTP WhatsApp) | Referrer mencatat sendiri pembayaran cash "titip" dari pelanggan yang ia referensikan lewat Portal Referrer → dapat komisi Titip (`CommissionRate.titip_amount`). HANYA untuk pelanggan yang punya `ppp_package_id` + rate Titip valid. Alur: layar konfirmasi detail → OTP 6-digit ke WhatsApp **Referrer sendiri** (event type WhatsApp ke-5 `referrer_action_otp`, penerima = `referrers.phone`) → verifikasi → baris `commission_ledger` langsung `scheme=titip status=eligible` (OTP = jaring pengaman, nominal dikunci ke rate — tidak ada approval admin, tidak ada input manual). `commission_ledger.payment_period` (date) untuk guard duplikat per-bulan (peringatan, bukan hard block). CREATE-ONLY (Referrer tidak bisa edit/hapus). Dashboard admin "Titip Masuk" (`/titip-masuk`, `commission_ledger.view`) = daftar kerja perpanjangan manual, BUKAN gate approval. **TIDAK ada otomasi NAS/RADIUS/MixRadius** — perpanjangan tetap manual admin. Plus fondasi: `CommissionScheme::Titip`, `ReferrerReferralResource` tampilkan SEMUA baris komisi (bukan `->first()`), Rekap Komisi portal diisi. **Amendment (2026-09-04)**: Fitur Titip diperluas — Portal Referrer sekarang menampilkan SEMUA pelanggan (bukan cuma yang direferensikan), "Catat Titip" tersedia untuk pelanggan mana pun dengan paket+rate valid (cash bisa dikumpulkan Sales/Teknisi/Agent), Rekap dipecah 2 tabel (Komisi vs Titip). Plus bonus fix sesi ini: CPE "offline palsu" (ambang online `CpeDeviceStatusSyncService` 5 menit → 3 jam config + scheduler `->runInBackground()`), link "Ganti Modem" ke detail device. **Ini iterasi, BUKAN final** — Portal Referrer akan dirombak lagi di sprint Manajemen User (v0.22.0) untuk unifikasi portal. | Selesai — merged `develop`→`main`, tagged `v0.9.6` (verifikasi manual detail di-skip Agung, portal akan dibongkar ulang di sprint Manajemen User) |
| v0.9.7  | Billing & Finance | Login Terpadu (Admin + Referrer, 1 pintu) | Gabung 2 halaman login terpisah (`/login` Fortify email, `/referrer/login` custom HP) jadi SATU pintu di `/login` — field pertama "Email atau Nomor HP". `Fortify::authenticateUsing()` deteksi email vs HP saat submit (email → jalur staff `users`; selain itu → jalur Referrer, reuse `App\Support\LoginIdentifierResolver`). Pesan gagal **identik** kedua jalur (tidak bocorkan identitas/jalur). `App\Http\Responses\LoginResponse` → `redirect()->intended('/')` (route `/` yang branch, `EnsureAdminPanelAccess::userHasAccess()`). Rate limit `throttle:login` (429) kedua jalur. `/referrer/login` GET → 302 `/login`, POST tetap kompat. Link "Lupa password?" (dari v0.9.6) dipindah ke form terpadu. **Branch `login-terpadu` dari `main`; dinomori `v0.9.7` (tag `v0.9.X` tanpa suffix, sesuai konvensi v0.9.1-v0.9.6).** | Selesai — merged `develop`→`main`, tagged `v0.9.7` |
| v0.9.8  | Billing & Finance | Perpanjang di Daftar Pelanggan + Fee Komisi + Multi-Bulan | 4 iterasi digabung 1 tag: **(1)** aksi "⚡ Perpanjang" dipindah dari Portal Referrer terpisah ke Daftar Pelanggan bersama (`customers.index`) — Referrer murni sekarang bisa akses route itu SAJA (`EnsureCustomerListAccess`, tampilan disederhanakan tanpa sidebar/tombol admin); OTP + ganti paket + komisi Titip untuk Sales/Freelance, `SubscriptionRenewalService::renew()`. **(2)** Tracking setoran: `commission_ledger.gross_amount`/`deposit_status` (belum_setor/sudah_setor) — Referrer pegang uang penuh, setor ke admin, komisi dibayar balik terpisah; ringkasan di Portal Referrer + admin. **(3)** Revisi "Titip Masuk" → **"Fee Komisi"** (label saja, route tetap), dikelompokkan 1 grup sidebar toggle-murni "Komisi" (+ "Rate Komisi"); checkbox selektif per baris "Tandai Sudah Setor (Terpilih)" (gantikan "Tandai Semua"); **BLOKIR KERAS** anti bayar-dobel per periode (`RuntimeException`, tanpa override aplikasi, dicek di awal buka modal). **(4)** Perpanjang Multi-Bulan (ADMIN ONLY, tidak pernah muncul di form Referrer) — preset 1/3/6/12 bulan, `commission_ledger.referrer_id` jadi nullable (baris "periode dibayar" tanpa Referrer tetap tercatat). Portal Referrer disederhanakan jadi Profil + Rekap Komisi + Rekap Titip saja (tabel pelanggan sudah pindah ke poin 1). Detail lengkap: CHANGELOG.md + CLAUDE.md. | Selesai — merged `develop`→`main`, tagged `v0.9.8` |
| v0.9.9  | Network/Komunikasi | WhatsApp Gateway Reliability | Investigasi keluhan disconnect/QR lambat: root cause DITEMUKAN via baca source Baileys langsung — race condition nyata di `connect()` (2 `await` sebelum socket terdaftar) bikin `makeWASocket()` terpanggil 2x untuk sesi yang sama pada window sempit (boot race / UI-refresh vs auto-reconnect timer) → 2 socket hidup sekaligus terdeteksi WhatsApp sebagai konflik → `device_removed`. **Fix**: `connectLocks` (in-flight promise per sesi) menutup race secara struktural; `getOrRefreshQr()` pada status `disconnected` sekarang benar-benar mempercepat reconnect (sebelumnya no-op total selama window backoff). Diverifikasi 2 lapis: isolasi murni (Baileys di-stub) + REAL terhadap container yang di-rebuild (2 request bersamaan → tepat 1 socket, round-trip sampai DB Laravel). Plus **Kode Pairing** (native Baileys `requestPairingCode`, alternatif scan QR — toggle per sesi di `/whatsapp-gateway`). Plus evaluasi WAHA murni dokumen (`docs/whatsapp-gateway-alternatif-evaluasi.md`) — rekomendasi TETAP Baileys custom (risiko WAHA "kehilangan sesi saat redeploy" terkonfirmasi via GitHub issue #1591, closed "not planned"). | Selesai — merged `develop`→`main`, tagged `v0.9.9` |
| v0.9.10 | Network/Komunikasi | Migrasi whatsmeow — Baileys Resmi Pensiun | Cutover total gateway WhatsApp dari Node.js/Baileys ke Go/whatsmeow. Baileys pensiun karena masalah upstream yang tidak bisa diperbaiki dari sisi kita (race condition `device_removed` berulang meski sudah difix `connectLocks` di v0.9.9 — sebagian besar kegagalan terakhir justru bug wiring UI/HMAC kita sendiri, bukan murni Baileys). whatsmeow terbukti stabil (~4 jam observasi tanpa disconnect, pesan nyata terkonfirmasi diterima). Kontrak HMAC/webhook/endpoint dipertahankan identik (lihat `docs/whatsapp-gateway-api-surface.md`) — Laravel-side (`WhatsappSessionService`/`SendWhatsappMessageJob`/dll) nyaris tidak berubah selain config key. Folder `whatsapp-gateway-go/` di-rename jadi `whatsapp-gateway/` (satu-satunya gateway sekarang); folder Node lama + container lama dihapus total. Persistensi sesi pindah ke database `whatsmeow_store` (Postgres, terpisah dari `boss_db`) — auto-reconnect tanpa scan ulang QR terverifikasi lintas beberapa kali recreate container. Detail lengkap: CLAUDE.md section "Migrasi whatsmeow". | Selesai — merged `develop`→`main`, tagged `v0.9.10` |
| v0.9.11 | Billing & Finance | Payout Komisi — Instan (Titip) & Batch Bulanan (Jendela per Rate) | Bagian "Payment" dari scope Commission (v0.9.0) yang belum pernah dibangun — v0.9.5 cuma menangani Eligibility (Pending→Eligible), bukan pembayaran aktualnya. **(1) Titip** — payout instan per transaksi/per grup Referrer (`/titip-masuk`), guard keras `deposit_status=sudah_setor` + wajib upload bukti bayar, tanpa jadwal. **(2) Komisi bulanan** (`recurring`/`limited_count`) — payout batch per Referrer (`/payout-komisi-bulanan`), jendela tanggal payout **dikonfigurasi per `CommissionRate` (per paket)** — `payout_window_start_day`/`_end_day`, NULL = kapan saja, diisi = hanya di rentang itu; tiap baris ledger dicek independen lewat rate paketnya (`CommissionPayoutService::isRowPayableNow()`), guard di server bukan cuma UI. Status transisi `Eligible`→`Paid` (`paid_at`/`paid_by`/`payment_proof_path`). Termasuk pembersihan data test (`test-daftar` + fixture anomali) + backup. | Selesai — merged `develop`→`main`, tagged `v0.9.11` |
| v0.9.12 | Billing & Finance | Perpanjang → Invoice ASLI + Lunas + Cetak 2 Tipe | Koreksi arah (keputusan Agung): Invoice mulai BENAR-BENAR dipakai (BOSS App menggantikan MixRadius/SmartOLT). Aksi "Perpanjang" sekarang membuat **Invoice ASLI** per periode + langsung LUNAS lewat `InvoiceService::markPaid()` (satu-satunya jalur sah — **caller ke-3**: dulu PATCH manual v0.3.4 + webhook Xendit v0.3.5) → OTOMATIS men-trigger pematangan **Komisi Penjualan v0.9.5** tanpa logic komisi baru. **BUKAN** mengaktifkan `GenerateDueInvoices` (tetap off) — pembuatan invoice ON-DEMAND. `RenewalInvoiceService` pakai 1 baris `subscriptions` tersembunyi per pelanggan (`status=Cancelled`, invisible untuk GenerateDueInvoices) sebagai "vehicle" (`invoices.subscription_id` NOT NULL). **Checklist `customers.tax_billable`** (default off): off → `tax_total=0` + cetakan tetap tampil "PPN 0%"; on → tax engine v0.3.3 normal (`sell_price`=DPP, PPN di atas). Guard anti-duplikat: reuse invoice yang ada (idempoten `generateForPeriod` + unique `(subscription_id, period_start, period_end)`), tidak pernah invoice ke-2. **Cetak Invoice**: browser-print (nol dependency PDF baru), 2 tipe — Standar A4 + Thermal 58/80mm (`GET /invoices/{invoice}/print?format=&width=`). Titip TETAP terpisah. **Revisi gabungan (5 bagian):** (A) `markPaid(Invoice, bool $notifyCustomer=true)` — Perpanjang lewat `false` supaya multi-bulan tak kirim N WA "payment received"; PATCH manual + webhook Xendit tetap `true`. (B) Section "Invoice" di Detail Pelanggan (`CustomerShow`) — list invoice khusus pelanggan + tombol Cetak; menu `/invoices` global tetap ada untuk overview. (C) `CommissionRateIndex` sembunyikan paket `sell_price=0` (list + guard `edit/save`). (D) "Fee Komisi" (`/titip-masuk`) diperluas jadi Titip + Bulanan (filter "Jenis Komisi"), 4 kartu ringkasan, komisi Bulanan bisa dibayar dari sini kalau jendela payout paketnya terbuka (`CommissionPayoutService::payMonthlyRow()` baru, tanpa bukti bayar); halaman "Payout Bulanan" tetap ada. (E) Menu baru "Riwayat Pembayaran" (`/riwayat-pembayaran-komisi`, `CommissionPaymentHistory`) di grup sidebar "Komisi" — list `commission_ledger` `status=Paid` (Titip + Bulanan), filter tanggal/referrer/jenis, grafik bar bertumpuk per bulan pakai Chart.js yang sudah ada (`window.commissionPaidChart`). 1616 test hijau. | Selesai — merged `develop`→`main`, tagged `v0.9.12` |
...
| v0.12.0 | Network         | PPPoE Provisioning, Technician API & Template Konfig CPE | Provisioning kredensial PPPoE (`radcheck`) hasil instalasi teknisi — di luar scope v0.7.5 karena `work_order_devices` tidak punya link balik ke `radcheck`. Sekalian API/otorisasi resmi teknisi/bot WhatsApp submit device (menggantikan bridge CS manual sementara dari v0.7.5). **Template Konfig CPE:** matriks Paket × Tipe Modem yang menyimpan config WAN default per kombinasi (reuse struktur `RemoteWanConfig` per-template, bukan singleton global) dan mendorongnya ke device saat aksi Ganti Paket (push mode) — scope detail (skema tabel baru, mekanisme push, integrasi ke alur Perpanjang/PSB) menyusul saat decision-gate sprint (BOSS-003). | Selesai — merged `develop`→`main`, tagged `v0.12.0`. Cluster 7 sub-versi (v0.12.1-v0.12.7): PPPoE Provisioning ke `radcheck` (binding otomatis + manual), Technician-scoped API + model klaim Work Order (claim/scan device/OTP konfirmasi WhatsApp), Template Konfig CPE + Push Konfig manual/otomatis (Ganti Paket, PSB baru). Detail lengkap: CHANGELOG.md v0.12.0. |
| v0.13.0 | Komunikasi      | WhatsApp 2-Arah (Inbound + State Machine Percakapan) | Infrastruktur pesan masuk untuk `whatsapp-gateway` (kini Go/whatsmeow, bukan lagi Baileys — lihat CLAUDE.md "Migrasi whatsmeow") — sejak v0.4.0 baru outbound-only. Mencakup listener inbound message, state machine percakapan per sesi, routing pesan masuk ke handler yang sesuai. Fondasi/wajah percakapan untuk fitur yang sudah/akan dibangun API-first di modul lain (bot WhatsApp jadi consumer tambahan, bukan desain ulang) — TIDAK termasuk logic bisnis spesifik per modul, itu tetap di endpoint masing-masing. | In Progress — decision-gate desain sudah dilakukan (state Redis+audit Postgres di v0.13.2, auth teknisi via match phone di v0.13.3, WA-bot PSB di v0.13.4), prasyarat blocking "WO Auto-Dispatch Scheduling + Notifikasi" (v0.26.0) **Done** 2026-09-17 — v0.13.4 sendiri kemudian dipecah jadi 3 sub-versi (2026-09-21, decision-gate lanjutan): **v0.13.4.1 (Klaim WO via signed-link) Selesai**, **v0.13.4.2 (Aktivasi via signed-link) — Backlog, PAUSED** (menunggu prasyarat OMCI, v0.23.0) dan **v0.13.4.3 (Close WO via signed-link)** masih Backlog biasa, belum ada decision-gate detail — lihat catatan v0.13.4.1 di akhir baris ini. **v0.13.1 (listener pesan masuk + webhook mentah ke Laravel) Selesai** — merged `develop`→`main`, tagged `v0.13.1`. Cluster 5 potongan: listener Go/whatsmeow (`events.Message`, filter IsFromMe/IsGroup) + webhook `NotifyIncomingMessage` + tabel `whatsapp_incoming_messages` (simpan mentah, tanpa state machine); tab "Pesan Masuk" di WhatsApp Gateway UI + scoping `reseller_id` (resolve dari `session_key` saat webhook diterima, tidak pernah menolak webhook kalau reseller tidak ketemu); fix bug `ResellerContext` tidak persist antar request Livewire di `render()`/visibilitas tab (root cause: route internal Livewire `POST .../livewire/update` tidak pernah dibungkus middleware `reseller.context`, yang cuma jalan di route GET awal); fix bug KELAS SAMA di 4 action methods (`createSession`/`editTemplate`/`saveTemplate`/`resetTemplateToDefault`) yang masing-masing memanggil `app(ResellerContext::class)` sendiri; rename label UI "Antrian"→"Pesan Keluar" (nama internal `tab`/`setTab('antrian')`/route/tabel tetap tidak berubah). **4 perbaikan pasca-rilis** (bug LID, bug ResellerContext render+action methods, bug cegah-dobel-session, tombol Logout reseller) — lihat section "Perbaikan Pasca-Rilis v0.13.1" di bawah, BUKAN sub-versi/tag baru, murni patch atas tag `v0.13.1` yang sudah ada. **v0.13.2 (state machine percakapan inti) Selesai** — merged `develop`→`main`, tagged `v0.13.2`. Fondasi GENERIK (`App\Services\Whatsapp\WhatsappConversationStateService`) — belum ada business logic PSB/OTP spesifik (itu v0.13.3/v0.13.4): state aktif di Redis (`Cache::` facade, key `wa-state:{phone}:{scope}` + pointer `wa-active-scope:{phone}`, TTL 2 jam flat bukan sliding), audit trail terpisah append-only di Postgres (`whatsapp_conversation_logs`, direction inbound/outbound/system). **v0.13.3 (auth teknisi + fallback) Selesai** — merged `develop`→`main`, tagged `v0.13.3`. `App\Services\Whatsapp\WhatsappTechnicianAuthService::resolveAuthorizedTechnicianForActiveWorkOrder()` — guard reusable murni (belum ada business logic PSB, itu v0.13.4): `technicians.phone` (ternormalisasi) cocok `work_orders.technician_id` dengan status IN (assigned, in_progress). Pesan masuk yang tidak match state aktif (v0.13.2) di-routing 2 cabang baru: teknisi terautorisasi tanpa state aktif → balas "fitur sedang dikembangkan" (`WhatsappEventType::TechnicianFeaturePending`); tidak match apa pun → fallback generik lewat sesi "direct" (`WhatsappEventType::UnrecognizedMessageFallback`). Dua keputusan desain: edge case 1 teknisi dengan >1 WorkOrder aktif sekaligus → pilih yang `updated_at` PALING BARU (dilaporkan eksplisit, bukan dipilih diam-diam); guard tambahan `TechnicianStatus::Active` ditambahkan sebagai defense-in-depth (tidak diminta eksplisit di instruksi awal). Detail lengkap: CHANGELOG.md v0.13.1/v0.13.2/v0.13.3. **v0.13.4.1 (Klaim WO via signed-link) Selesai** — merged `develop`→`main`, tagged `v0.13.4.1`, 2026-09-21. Teknisi utama meng-klaim WO lewat signed URL public (`URL::temporarySignedRoute`, middleware `signed`, TANPA login/session sama sekali — signature itu sendiri = otorisasi) yang dikirim WhatsApp saat dispatch/reminder (`WorkOrderDispatchService::sendTo()`, link UNIK per teknisi), mencatat partner kerja (`work_order_claim_partners`, GENUINELY terpisah dari `work_order_technicians` v0.12.3), alat non-modem (`work_order_tool_usages`, quantity+siapa bawa) dan modem (`work_order_modem_units`, WAJIB per-unit serial_number+mac_address, tanpa stok) dalam SATU form/transaksi. **Klaim TIDAK PERNAH mengubah `WorkOrderStatus`** — cuma menambah metadata `work_orders.claimed_at`/`claimed_by_technician_id` (murni timestamp+FK). Submit kedua ke WO yang sudah diklaim (link lama masih valid, atau race 2 tab) DITOLAK — `lockForUpdate()` di dalam transaksi + `WorkOrderAlreadyClaimedException`, controller menampilkan halaman read-only (siapa klaim + data tersimpan), bukan form kosong/overwrite diam-diam. Reminder harian untuk WO yang sudah diklaim TETAP mengirim WA (bukan skip) tapi lewat event type BARU `WhatsappEventType::WorkOrderClaimedReminder` (dikonfirmasi eksplisit Agung sebelum dibuat, sesuai governance "no new WhatsappEventType without asking") — TANPA `claim_link` (sudah tidak relevan), HANYA ke teknisi yang genuinely klaim (bukan broadcast). CRUD sederhana master data `tool_types` (`App\Livewire\Installation\ToolTypeIndex`, `/tool-types`, seed 4 item: Dropcore/Adapter/Patchcore/Adaptor Modem per tenant) ikut dibangun di sub-versi ini. Verifikasi: scoped test (Installation 165/165, Whatsapp 160/161 — 1 gagal pre-existing tidak terkait), verifikasi manual browser DILEWATI atas izin eksplisit Agung (sedang pakai HP), dicatat sebagai pengecualian. **v0.13.4.2 (Aktivasi via signed-link) Backlog — PAUSED.** Aktivasi customer butuh ONU sudah online di level fiber (OMCI/OLT) sebelum GenieACS/PPPoE provisioning bisa jalan sama sekali. Dipause 2026-09-21 setelah decision-gate PPPoE/WAN push CT-COM selesai diinvestigasi (temuan lengkap: skema PPPoE `{cid}@ppp.bajastu.id`/password tetap, pool-slot-preset GenieACS untuk hindari masalah cache preset baru, UX async karena siklus cache ~5,5 menit — semua sudah didesain, tinggal dieksekusi begitu OMCI selesai). Lanjut setelah cluster OMCI (`v0.23.0`) di-tag. **v0.13.4.3 (Close WO via signed-link) Backlog** — link penutupan WO terpisah menyusul, belum ada decision-gate. |
| v0.14.1 | Network         | Bandwidth Profile              | Fondasi cluster "Profil Paket" (terinspirasi MixRadius V3.2, 7 sub-versi). Tabel `bandwidth_profiles` — profil reusable upload/download min-max, disimpan internal dalam Kbps terlepas satuan input user. REST API + Livewire `/bandwidth-profiles` | Selesai — merged + tagged `v0.14.1` |
| v0.14.2 | Network         | IP Pool Pelanggan              | Konsep IP pool BARU untuk alokasi IP end-device pelanggan hotspot/PPP — beda total dari `VpnIpPool` (v0.6.2, itu untuk tunnel VPN NAS↔BOSS App). Tabel `customer_ip_pools`, wajib terikat `nas_id`. REST API + Livewire `/customer-ip-pools` | Selesai — merged + tagged `v0.14.2` (tag ini juga mencakup v0.14.2.1 + v0.14.2.2) |
| v0.14.2.1 | Network     | RouterOS Live-Push (mulai dari IP Pool) | **Dimajukan dari rencana semula v0.14.6** — kemampuan live-push RouterOS API PERTAMA di codebase ini, dimulai khusus untuk `CustomerIpPool` (`/ip pool add/set/remove`, lookup via comment stabil, bukan `name`). Job async (`PushCustomerIpPoolToMikrotikJob`/`RemoveCustomerIpPoolFromMikrotikJob`), retry 3x dengan backoff 30s/2min/5min, kolom `mikrotik_sync_status`/`mikrotik_synced_at`/`mikrotik_sync_error`, badge + tombol "Sync Ulang" di UI. **Sengaja TIDAK digeneralisasi ke entity lain di sub-versi ini** — pola ini jadi cetak biru untuk v0.14.6 nanti (Bandwidth Profile/Grup Profil/Profil Hotspot/Profil PPP). Diverifikasi nyata end-to-end terhadap `ro-hotspot.bajastu.id` (push/edit/delete/simulasi-gagal-lalu-retry-otomatis-berhasil, semua dikonfirmasi lewat pembacaan `/ip pool print` router asli) | Selesai — folded ke tag `v0.14.2` |
| v0.14.3 | Network         | Grup Profil                    | Konfigurasi RADIUS/Mikrotik group per-NAS (tipe Hotspot/PPP), link ke IP Pool (v0.14.2)/DNS server/parent queue. Tabel `network_profile_groups` + live-push RouterOS (`/ppp profile` untuk PPP; update `address-pool` server Hotspot existing untuk Hotspot) + tulis `radgroupreply` (keputusan eksplisit Agung) | Selesai — merged + tagged `v0.14.3` (tag ini juga mencakup v0.14.3.1) |
...
| v0.14.5.2 | Network       | Revisi Profil PPP — Masa Aktif 0 = Unlimited | Form Profil PPP (v0.14.5) menolak Masa Aktif = 0; referensi MixRadius mengizinkan 0 = "UNLIMITED". Investigasi: satu-satunya downstream consumer `active_duration_value` adalah `PppPackage::routerOsSessionTimeout()` — TIDAK ADA perhitungan tanggal expired pelanggan di codebase (billing manual/MixRadius). Fix: `routerOsSessionTimeout()` jadi `?string`, return `null` saat value=0 (session-timeout tidak dikirim ke `/ppp profile`, sama posture Profil Hotspot Unlimited); `PppPackage::isUnlimitedDuration()`; validasi `min:1`→`min:0` di 4 tempat (Store/Update Request + Livewire create/edit), negatif tetap ditolak; form hint "0 = Unlimited"; list tampil "Unlimited"; `is_unlimited_duration` di API resource. Profil Hotspot TIDAK diubah (sudah punya toggle `profile_type=unlimited`). **INSIDEN CLOSURE**: branch ini sempat menggantung — branch berikutnya (`investigasi-nama-paket-vs-grup`) dibuat "dari main" tanpa cek → Agung kena regresi validasi ">= 1" muncul lagi. Ditutup bersama perkuatan aturan closure di CLAUDE.md. | Selesai — merged `develop`→`main`, tagged `v0.14.5.2` |
| v0.14.5.3 | Network       | Aturan Nama Profil Paket — dunia PPP bebas senama + auto-differentiate router | Ralat validasi collision nama v0.14.5. **DI DALAM dunia PPP** (IP Pool ppp/general, Grup Profil ppp, Profil PPP) nama BOLEH sama semua — sengaja, konsistensi WinBox untuk troubleshooting. `PppPackage::collidesWithExistingName()` tidak lagi blokir nama = Grup Profil ppp / Profil PPP lain; HANYA blokir bentrok dunia HOTSPOT (Grup Profil tipe hotspot / Profil Hotspot) di NAS sama. Collision `/ppp profile` di RouterOS di-handle otomatis: `PppPackage::routerOsProfileName()` kirim verbatim kecuali bentrok → `"{nama} (pkg #{id})"` (kolom DB `name` tidak berubah); `NetworkProfileGroupService` re-dispatch push Profil PPP senama sebelum push Grup Profil-nya. Hotspot↔PPP untuk IP Pool sudah terjaga oleh unique `(nas_id, name)`. Bandwidth Profile bebas (tidak push objek RouterOS bernama). Aturan lengkap permanen di CLAUDE.md "ATURAN NAMA PROFIL PAKET — FINAL". Branch `investigasi-nama-paket-vs-grup` — awalnya investigasi murni, lalu eksekusi. | Selesai — merged `develop`→`main`, tagged `v0.14.5.3` |
| v0.14.5.4 | Network       | RouterOS Live-Push — `local-address`/Ensure IP Pool + ATURAN KERAS 1:1 Grup Profil⇄Profil PPP + auto-differentiate nama Pool | **BUG #1**: `customer_ip_pools.gateway_ip` (disimpan sejak v0.14.2 khusus untuk `/ppp profile local-address`) tidak pernah dikirim → PPP klien connect tapi tak dapat routing. **BUG #2**: push `/ppp profile` yang mereferensikan `/ip pool` TANPA pastikan pool ada dulu → RouterOS tolak "invalid value for argument remote-address:" → resync gagal PERMANEN setelah admin hapus pool manual. Fix: `syncIpPool()` idempoten by-comment dipanggil DULU; `$pool->gateway_ip` di-resolve live sebagai `$localAddress`. **Bagian A (ATURAN KERAS 1:1)**: 1 Grup Profil = 1 Profil PPP aktif (satu `/ppp profile` cuma bisa 1 rate-limit) — partial unique index + validasi + dropdown exclude; Profil PPP tidak lagi push `/ppp profile` sendiri, ia UPDATE objek milik Grup Profil (`App\Services\Network\PppProfileSyncService` baru, dipakai bareng kedua Job); Profil PPP dihapus/nonaktif → profile Grup Profil di-RESET ke bare (`rate-limit=""`); `routerOsProfileName()` v0.14.5.3 dihapus. **Bagian B**: `CustomerIpPool::routerOsPoolName()` auto-suffix `" (pool)"` kalau nama Pool bentrok nama `/ppp profile` di NAS sama (bug WinBox: RouterOS salah resolve `remote-address`). Diverifikasi nyata di `ro-hotspot.bajastu.id`; `test-x86-bajastu` tidak disentuh. **+ DARURAT (sesi sama)**: `RouterOsQueuePriority::toRateLimitString()` grup burst `"1s/1s"` (dari v0.14.5.1) DITERIMA `/ppp profile/set` tapi GAGAL saat sesi PPPoE connect (`could not add queue: no download-burst-time`) — ~200 sesi pelanggan `ro-hotspot` terputus. Fix: format `rate-limit` POLOS `"{up}k/{down}k"` (priority jadi "stored, not pushed"); `composeRateLimit()` baru = jalur burst all-or-nothing yang benar. Diverifikasi live: log berhenti banjir, sesi reconnect. | Selesai — merged `develop`→`main`, tagged `v0.14.5.4` |
...
| v0.21.0 | Operasional     | Template Voucher Cetak (Token/Hotspot) | **Reservasi slot nomor + nama saja — scope detail BELUM ditentukan.** Sistem template untuk cetak fisik kode voucher Token/Hotspot — terinspirasi MixRadius V3.2 (referensi lengkap sudah didokumentasikan dari screenshot + kode HTML Agung). Mencakup: CRUD template (list, enable/disable, edit, preview, hapus); editor HTML mentah dengan daftar variabel yang bisa dipakai (username, password, harga paket, profil, bandwidth, validity, time limit, quota limit, tipe, nama perusahaan, nomor HP, URL hotspot — pola MixRadius, sintaks variabel BOSS App sendiri menyusul saat scoping); alur pilih voucher (checkbox multi-select) → pilih template → cetak; kirim voucher via WhatsApp (opsional); **minimal 1 template contoh siap pakai** dibuat tim BOSS App (kartu voucher dengan QR code, logo, harga, kode, footer — referensi desain dari HTML/CSS Agung). Kemungkinan terhubung ke `HotspotPackage` (v0.14.4) sebagai sumber data paket — tapi entity "voucher/token individual hasil generate" **belum ada di codebase** (beda dari `HotspotPackage` yang cuma definisi paket), kemungkinan besar perlu dibangun juga sebagai bagian sprint ini. Decision-gate scope lengkap terpisah saat sprint ini benar-benar dimulai (BOSS-003) — jangan asumsikan detail apa pun dari baris ini | Backlog |
| v0.22.0 | Operasional     | Manajemen User (Staff CRUD + Referral Terpadu) | Cluster CRUD Staff internal (tabel `users` + role Spatie) sekaligus rombakan lanjutan Portal Referrer menuju unifikasi. Branch lama `manajemen-user` (dari `main`, 2026-09-02) DITINGGALKAN sebagai basis kerja (stale ~2 minggu, cuma 1 commit dokumentasi, nol kode genuinely dibangun) — dikerjakan ulang dari `main` di branch `v0.22.1-staff-crud`. Sub-versi terkunci — **urutan direvisi 2026-09-15** (gara-gara insiden Kamisem di bawah + keputusan login-via-HP; jangan asumsikan nomor sub-versi dari catatan lama di bagian lain file ini, baris ini yang final): **v0.22.1 "Staff CRUD Dasar"** — migration `users.is_disabled`+`users.phone` (nullable, HP di sini masih sekadar data kontak, BELUM jadi alat login), `UserPolicy` 4 method terpisah (viewAny/view/create/update/delete), `StaffService` (create dengan password acak show-once, update tanpa sentuh password, disable/enable, delete dengan guard eksplisit ke `reseller_users`/`technicians`/`cpe_action_logs` — 3 relasi nyata yang kalau tidak di-guard akan cascade/RESTRICT diam-diam), blokir login akun disabled di KEDUA entry point (`Fortify::authenticateUsing()` + legacy `ReferrerLoginController`), semua 9 role Spatie termasuk superadmin bisa dipilih — **Selesai, tagged `v0.22.1`**; **v0.22.2 "Login via Nomor HP + Email Opsional"** — `users.phone` naik jadi alat login utama (`NOT NULL`+`UNIQUE`, `users.email` jadi nullable), `LoginIdentifierResolver::resolvePhoneUser()` (cek `users.phone` dulu, fallback `referrers.phone`) dipakai bareng login utama (`/login`) dan jalur kompat lama (`/referrer/login`), `StaffService`+`StaffIndex` phone wajib/email opsional. Fix susulan: migration data menormalisasi ulang `users.phone` akun LAMA yang belum pernah lewat normalize-on-save (login via HP untuk akun existing tadinya selalu gagal apa pun format yang diketik — root cause: normalize-on-save tidak retroaktif ke baris yang sudah ada) — **Selesai, tagged `v0.22.2`**; **v0.22.3 "Checkbox 'Jadikan juga Referrer' + Link/Unlink Otomatis"** (dulu bernomor v0.22.2 sebelum revisi urutan ini) — checkbox di form create Staff, muncul reaktif cuma untuk role `sales_internal`/`sales_freelance`/`customer_service`/`teknisi`, `ReferrerService::createAndLinkToStaff()` (reuse `linkExistingUser()`, guard collision `(tenant_id, phone)` eksplisit sebelum insert — pesan jelas, staff tetap berhasil dibuat meski link Referrer gagal), disable/enable staff cascade ke status Referrer ter-link, delete staff dengan Referrer ter-link tetap boleh (Referrer tidak ikut hilang, `user_id` jadi null via `nullOnDelete()` yang sudah ada sejak awal) — **Selesai, tagged `v0.22.3`**; **v0.22.4 "Auto-kirim Password ke WA/Email"** (dulu bernomor v0.22.3) — `StaffService::create()` otomatis mengirim 2 pesan WA terpisah (pengantar + password polos, event type baru `StaffInitialPasswordNotice`/`StaffInitialPasswordValue`, disetujui eksplisit — password polos SENGAJA tidak pernah lewat sistem Template WA sama sekali, dijamin struktural tidak bisa dirusak admin lewat edit template), plus fitur baru "Lupa Password" khusus staff (`/staff/forgot-password`, `StaffActionOtpService`, reuse event type `ReferrerActionOtp` — pola persis Lupa Password Referrer) — **Selesai, tagged `v0.22.4`**; **v0.22.5 "Full Regression + Close Cluster"** (penutup cluster v0.22.0, dulu bernomor v0.22.4) — full regression suite dijalankan penuh (2101 test, 0 gagal), Pint full-codebase bersih (3 isu pre-existing di file tak disentuh cluster ini, tidak diperbaiki), 0 TODO/FIXME tertinggal di 33 file yang disentuh v0.22.1-v0.22.4 — **Selesai, tagged `v0.22.5`**. **v0.22.6 "Wajib Ganti Password (First-Login) + Cleanup Akun"** (2026-09-17, REOPEN cluster v0.22.0 untuk 1 patch kecil, branch `v0.22.6-force-password-change` dari `main`) — kolom `users.must_change_password` (boolean, default false); `StaffService::create()` set `true` untuk SETIAP staff baru (password-nya selalu random-generated); `App\Http\Middleware\EnsurePasswordChanged` (alias `password.changed`, dipasang HANYA di grup `['auth','admin.panel']` — TIDAK PERNAH di grup `customers.list`/`referrer.portal`, jalur Referrer sama sekali tidak terpengaruh) memaksa redirect ke halaman `/change-password` (`App\Livewire\Auth\ChangePassword`) sampai user mengganti password-nya sendiri; `StaffForgotPassword::submitPassword()` (reset via OTP) set `false` — user sudah pilih sendiri, tidak perlu dipaksa ganti lagi. Form `/change-password` SENGAJA cuma "Password Baru" + "Ulangi Password Baru" — TANPA "Password Saat Ini" (dikonfirmasi eksplisit Agung: user sudah lolos autentikasi normal untuk sampai ke halaman ini, re-input password lama cuma nambah friksi tanpa nilai keamanan tambahan; beda dari `StaffForgotPassword` yang jalur GUEST dan verifikasi identitasnya lewat OTP WhatsApp). Plus cleanup akun: SEMUA 8 user existing (termasuk 2 akun "TEST - Jangan Hapus" — guard `StaffService::delete()` TIDAK memblokir keduanya, karena baris `Technician` terkait sudah dihapus di sesi cleanup closure v0.26.0 sebelumnya) dihapus lewat `StaffService::delete()` (guard tetap aktif, tidak di-bypass), lalu akun baru Agung Haryanto dibuat manual (bukan lewat `StaffService::create()` — email/phone di-reuse sama persis, role `superadmin`, password fixed `"password"`, `must_change_password=true`, SENGAJA TIDAK lewat 2 pesan WA otomatis karena password-nya bukan random). **Diverifikasi manual Agung** (login pertama → langsung `/change-password` → ganti password → lanjut dashboard normal) — **Selesai, tagged `v0.22.6`** (103 test Auth-terkait hijau, Pint bersih). **Backlog tersisa dari cleanup ini**: staff teknisi (Sahrul/Yusuf/Firman) yang ikut terhapus perlu dibuat ulang lewat `/staff` untuk kelanjutan testing broadcast WA teknisi di cluster v0.26 (lihat entry Backlog gap `technicians` di bawah). **v0.22.7 "Lock Referral Pelanggan saat Staff-Referrer Dihapus"** (2026-09-20, REOPEN cluster v0.22.0 lagi, branch `v0.22.7-referral-lock-on-delete` dari `main`) — kolom `customers.referral_locked` (boolean, default false) + `customers.locked_former_referrer_name` (string, nullable). `StaffService::delete()` (dalam TRANSAKSI yang sama dengan hapus staff, method baru `lockCustomerReferralsForDeletedStaff()`) mengunci setiap `Customer` yang mengarah ke Referrer ter-link staff yang dihapus: `referred_by_referrer_id` genuinely dikosongkan (bukan dibiarkan nyantol ke Referrer tanpa login), nama Referrer lama disimpan sebagai jejak. `CustomerShow::referrerLocked()` diperluas (JUGA `true` kalau `referral_locked`, bukan cuma `referred_by_referrer_id` terisi) + `updateCommissionAttribution()` menolak EKSPLISIT (addError, bukan diam-diam diabaikan) percobaan pindah referral untuk customer yang `referral_locked`. Tampilan panel "Paket & Referral" (mode readonly maupun edit) menunjukkan "Referral: (kosong) — sebelumnya oleh {nama}, terkunci karena staff resign, tidak bisa dipindahtangankan" (styling muted). **Forward-only, TANPA migrasi data retroaktif** — dikonfirmasi Agung: kasus lama Kamisem (insiden v0.22.1 di bawah) sudah dibereskan manual sebelum kolom ini ada, dan diverifikasi ulang di Langkah 0 (0 customer nyantol ke referrer id manapun yang tersisa saat itu). **Diverifikasi manual Agung — Selesai, tagged `v0.22.7`** (42 test scoped + 239 test regresi lebih luas hijau). **v0.22.8 "Auto-cleanup Referrer Orphan Kosong + Delete Guard Ketik-Nama"** (2026-09-20, sama-hari, branch `v0.22.8-referrer-collision-ui` dari `main` — SIBLING v0.22.7, bukan turunannya, dikonfirmasi lewat investigasi graph terpisah sebelum closure) — 2 bagian independen: **(A)** `ReferrerService::createAndLinkToStaff()` — collision `(tenant_id, phone)` sekarang dibedakan: Referrer orphan (`user_id` null) DAN genuinely 0 data nyantol (0 `commission_ledger`, 0 `customers`) → row lama **DIHAPUS PERMANEN** (hard delete, `Referrer` tidak pakai SoftDeletes), staff dapat Referrer BARU yang fresh (id beda dari yang lama) — staff TIDAK PERNAH mewarisi identitas/histori Referrer lama; orphan TAPI ada data nyantol, ATAU sudah taken (`user_id` user lain) → hard block, SATU pesan generik yang sama untuk kedua sub-kasus ("...nomor HP ini sudah pernah dipakai dan masih ada data terkait / masih aktif dipakai user lain..."), row lama tidak disentuh. `commission_ledger.referrer_id` adalah `cascadeOnDelete()` (bukan RESTRICT) — pengecekan "0 data nyantol" di kode aplikasi karena itu WAJIB benar, bukan sekadar optimisasi. **(B)** Delete guard `StaffIndex` diganti dari `wire:confirm` native browser (dikonfirmasi Langkah 0: MASIH UTUH, tidak pernah ke-drop) jadi modal ketik-nama — tombol "Hapus Permanen" disabled sampai input PERSIS cocok nama staff (case-sensitive), berlaku sama untuk semua staff tanpa pengecualian, defense-in-depth server-side kalau nama tidak cocok (modal tetap terbuka + error, bukan silent no-op). Iterasi PERTAMA sub-versi ini sempat menawarkan tombol "Link ke Referrer lama" untuk kasus orphan — DIGANTI TOTAL oleh (A) di atas sebelum closure (staff baru tidak boleh mewarisi identitas Referrer lama sama sekali). **Selesai, tagged `v0.22.8`** (227 test hijau filter Staff). Simulasi `git merge-tree` mengonfirmasi kedua sibling branch (v0.22.7/v0.22.8) bisa digabung tanpa konflik sebelum closure dieksekusi — satu-satunya file overlap (`StaffServiceTest.php`) auto-merge bersih. **Insiden nyata v0.22.1 (2026-09-15)**: akun Referrer produksi lama ("Kamisem") sempat terhapus permanen lewat `/staff` karena halaman itu awalnya menampilkan SEMUA baris `users` tanpa filter role (akun portal zero-role ikut muncul, tanpa tanda visual apa pun) — root cause ditutup (`StaffIndex::render()` sekarang `->whereHas('roles')`), 7 baris `customers.referred_by_referrer_id` yang mengarah ke akun itu di-unlink manual (nol pelanggan hilang, diverifikasi count sebelum/sesudah), row `referrers` dihapus permanen atas konfirmasi eksplisit Agung setelah dipastikan tidak ada data pelanggan yang ikut hilang. Lihat CLAUDE.md untuk kronologi investigasi penuh. | Selesai — v0.22.1 (tag `v0.22.1`), v0.22.2 (tag `v0.22.2`), v0.22.3 (tag `v0.22.3`), v0.22.4 (tag `v0.22.4`), v0.22.5 (tag `v0.22.5`, penutup cluster — full regression 2101 test hijau), v0.22.6 (tag `v0.22.6`, 2026-09-17, patch reopen — wajib ganti password login pertama + cleanup akun, 103 test Auth-terkait hijau), v0.22.7 (tag `v0.22.7`, 2026-09-20, patch reopen — lock referral pelanggan saat staff-Referrer dihapus), v0.22.8 (tag `v0.22.8`, 2026-09-20, patch reopen — auto-cleanup Referrer orphan kosong + delete guard ketik-nama staff, 227 test hijau) — 8 sub-versi total |
| v0.23.0 | Network | OMCI Integration untuk Zero-Touch ONT Provisioning | **Cluster dimulai — scope detail `v0.23.2` dan seterusnya BELUM ditentukan, decision-gate terpisah.** Terpisah TOTAL dari TR-069/GenieACS (sudah dibangun sejak v0.7.x) — OMCI dipakai ONT yang **tidak** support TR-069. **BOSS App integrasi LANGSUNG ke OLT** (CLI native vendor: HSGQ, ZTE, dll) — **TIDAK lewat SmartOLT sebagai perantara/proxy**. **Keputusan terkunci (v0.23.1, 2026-09-21 — MENGGANTIKAN catatan lama "SmartOLT tetap dipakai paralel" di baris ini)**: (1) **SmartOLT dibuang PENUH dari BOSS App** — bukan lagi dipakai paralel untuk kebutuhan apa pun; provisioning ONT baru ke depan sepenuhnya lewat integrasi native OLT (CLI/API vendor langsung); (2) **jalur jaringan produksi** = sidecar OLT dedicated via WireGuard, **dikerjakan di `v0.23.2`** (bukan v0.23.1 — riset CLI v0.23.1 memakai jalur sementara lewat network namespace `librenms-dispatcher` yang sudah ada, bukan jalur produksi); (3) **akses CLI riset** = read-only dengan allowlist command per-OLT (bukan tulis sembarangan, tanpa `save`), kredensial tidak pernah dicetak/dikutip/disimpan di mana pun. **Gambaran besar 3 jalur provisioning ONT ke depan**: (1) **TR-069** via GenieACS — sudah ada (v0.7.x); (2) **OMCI** via integrasi native OLT — sprint ini; (3) **DHCP Option 43** — solusi sementara effort rendah, sudah disinggung sebelumnya. **`v0.23.1` "Riset CLI OLT" SELESAI (2026-09-21, verifikasi manual Agung)** — dokumen referensi CLI read-only `docs/omci/hsgq-e04id-cli-reference.md` (HSGQ-E04ID/EPON) dan `docs/omci/hsgq-g02id-cli-reference.md` (HSGQ-G02ID/GPON); item keamanan yang ditemukan selama riset (kredensial sementara, SNMP community) dicatat di `docs/omci/backlog-security.md`, belum dieksekusi. **Urutan sub-versi TERKUNCI (keputusan Agung, 2026-09-21)**: `v0.23.2` Riset CLI ZTE C300, `v0.23.3` Sidecar OLT via WireGuard, `v0.23.4` Registry ONU, `v0.23.5` Aktivasi ZTE, `v0.23.6` Aktivasi G02ID, `v0.23.7` Aktivasi E04ID, `v0.23.8` Orkestrator, `v0.23.9` Penutupan cluster — **nama slot saja, scope detail per sub-versi (`v0.23.3` dan seterusnya) BELUM ditentukan**, jangan diasumsikan dari baris ini. **`v0.23.2` "Riset CLI ZTE C300" SELESAI (2026-09-22, verifikasi manual Agung)** — dokumen referensi CLI read-only `docs/omci/zte-c300-cli-reference.md` (login/mode, tabel command TERUJI/DARI REFERENSI PUBLIK/BELUM DIUJI, urutan konfigurasi aktivasi nyata dari 2 ONU uji Agung — `onu type sn` di level PON, `tcont`/`gemport`/`service-port`/VLAN di level interface ONU, `pon-onu-mng` untuk flow/vlan-filter/akses manajemen termasuk TR-069 lewat allowlist `security-mgmt` generik — BUKAN blok `tr069-mgmt` terpisah seperti dugaan referensi publik awal, tabel profil paket tcont+traffic 9 profil bernama selaras `PppPackage` BOSS App, katalog 28 tipe ONU bawaan firmware). Item keamanan tambahan dari riset ini (password akun telnet SmartOLT di ZTE, akun BOSS terbatas belum dibuat, kemungkinan port-forward Telnet/SNMP ke IP publik NAS, peringatan "password tidak kuat" dari device) dicatat di `docs/omci/backlog-security.md`, belum dieksekusi. **Format identitas ONU TERKUNCI**: `"Nama - CID"` — nomor HP TIDAK LAGI ditulis di field nama/desc OLT (mengoreksi pola lama yang teramati di riset v0.23.1, `"Nama - NomorTelepon"`); perubahan massal ONU LAMA yang masih pakai pola nomor-HP adalah sub-versi terpisah nanti, TIDAK termasuk cluster ini. Nomor slot `v0.23.0` — `v0.22.0` ("Manajemen User") didaftarkan di branch `manajemen-user` (belum di-merge saat baris ini ditulis). | In Progress |
| v0.24.0 | Network         | Redundansi WireGuard Node — Server Fisik Terpisah | **Reservasi slot nomor + nama saja — scope detail BELUM ditentukan, decision-gate terpisah.** Server VPS kedua (`160.25.201.205`) sudah diinvestigasi dan siap (Docker belum terinstall, network bersih, latency ~0.5ms ke server utama) tapi BELUM dipasang node WireGuard — arsitektur pool VPN sekarang (sticky `/30` per NAS, fragment+reconcile — lihat CLAUDE.md bagian "WireGuard /30 Per-NAS Tunnel Blocks"/"Fragment+Reconcile Routing") berasumsi semua node berada di **1 Docker host yang sama**, perlu desain ulang untuk node lintas-host (routing antar host, bukan sekadar tambah container `wireguard-nodeN` baru). Terkait juga catatan lama "OSPF Dynamic Routing — DISABLED" (lihat CLAUDE.md) yang eksplisit menyebut kondisi ini sebagai pemicu untuk dipertimbangkan ulang: "kalau/ketika node container ini ever actually move to separate PHYSICAL servers... real routing protocol convergence... jadi real requirement fragment+reconcile's simple polling loop can't cleanly solve." Scope detail belum ditentukan — decision-gate terpisah saat sprint ini benar-benar dimulai (BOSS-003) — jangan asumsikan detail apa pun dari baris ini | Backlog |
| v0.25.0 | Operasional     | Arsitektur Multi-Tenant SaaS — Isolasi per Pelanggan BOSS App | **Reservasi slot nomor + nama saja — INI KEPUTUSAN ARSITEKTUR FUNDAMENTAL, direkomendasikan jadi prioritas pembahasan strategis SEBELUM banyak fitur baru lain dibangun di atas asumsi arsitektur yang belum final — bukan keputusan cepat, perlu sesi perencanaan serius tersendiri.** Keputusan model bisnis BOSS App sebagai produk dijual ke ISP lain. Referensi model Agung: MixRadius — 1 deployment, banyak penyewa dibedakan subdomain saja, fitur identik. BOSS App sudah punya fondasi `tenant_id` di hampir semua tabel (mendukung pola serupa untuk lapisan aplikasi web/CRM/billing/komisi). **TAPI ada komplikasi nyata yang mungkin tidak dihadapi MixRadius**: BOSS App langsung mengelola infrastruktur jaringan fisik (FreeRADIUS, WireGuard, GenieACS) — berbagi instance yang sama antar ISP yang tidak saling kenal berisiko (keamanan data RADIUS lintas-tenant, tabrakan skema IP internal masing-masing ISP — persis kelas masalah yang sudah pernah dialami sendiri di insiden WireGuard IP collision, lihat CLAUDE.md). Kemungkinan besar arsitektur yang tepat = **hybrid**: aplikasi web+database shared (pola subdomain seperti MixRadius), TAPI infrastruktur jaringan (FreeRADIUS/WireGuard/GenieACS) tetap terisolasi per-tenant (desain detail belum ditentukan — bisa container terpisah per tenant, atau namespace yang sangat hati-hati). Terkait juga dengan `v0.19.0` (Feature Toggle/Module Management, sudah terdaftar) — kemungkinan saling melengkapi. Scope detail belum ditentukan — decision-gate terpisah saat sprint ini benar-benar dimulai (BOSS-003) — jangan asumsikan detail apa pun dari baris ini | Backlog |
| v0.26.0 | Installation    | WO Auto-Dispatch Scheduling + Notifikasi (Prasyarat v0.13.4) — **Done** | **PRASYARAT BLOCKING untuk v0.13.4 (WA-bot PSB), BUKAN backlog biasa yang bisa ditunda tanpa batas.** Dicatat dari planning chat v0.13.0, 2026-09-15, hasil decision-gate v0.13.4 yang menemukan gap ini. Domain Installation/WorkOrder — BUKAN scope WhatsApp, WA cuma efek samping. Mekanisme yang dibutuhkan (dijelaskan Agung): WO "keluar" (dispatch, terlihat/aktif ke teknisi) mengikuti jadwal — kalau calon pelanggan punya janji hari+jam kunjungan spesifik (sudah diinput sales/admin) → WO dispatch 2 jam sebelum janji itu; kalau TIDAK ada janji spesifik → WO keluar segera, tidak peduli hari/jam. Reminder: kalau WO masih belum dikerjakan (status belum Completed) keesokan harinya → dispatch ulang/notifikasi ulang jam 8 pagi. "Keluar"/dispatch memicu 2 notifikasi WA: (a) japri ke teknisi yang di-assign, (b) notifikasi ke grup WA. **Temuan investigasi teknis (2026-09-15, dicatat verbatim supaya sprint berikutnya tidak mengulang investigasi)**: kolom `work_orders.scheduled_at` SUDAH ADA di skema (migration `2026_08_03_170003`) tapi genuinely dead — tidak ada satu pun UI/API/Service yang pernah mengisi nilainya (`StoreWorkOrderRequest` dan `AssignWorkOrderRequest` tidak menerima field ini sama sekali); tidak ada scheduled command/cron yang cek WO berdasarkan janji — 21 file di `app/Console/Commands/` dicek satu-satu, nol yang relevan; `WhatsappEventType` belum punya case untuk "WO ditugaskan/dispatch ke teknisi" — infrastruktur generik `WhatsappGatewayService` sudah ada dan reusable, tapi event type + titik pemanggilannya (kemungkinan besar di `WorkOrderService::assignTechnician()` atau command baru) belum dibuat; kirim WA ke GRUP belum ada kapabilitasnya sama sekali di level gateway Go — bukan sekadar "belum di-wire" di Laravel, `whatsapp-gateway/` (Go/whatsmeow) saat ini SELALU membangun JID individu (`@s.whatsapp.net`), tidak ada jalur kirim ke JID grup (`@g.us`) di mana pun, baik sisi Go maupun `WhatsappPhone::normalize()` Laravel — ini butuh pengembangan baru di gateway, bukan reuse; CLAUDE.md section v0.5.0 sudah lama mencatat gap generik ini ("automatic WhatsApp notifications for work order events... wasn't wired up this sprint") — belum pernah diambil sprint mana pun sejak saat itu. Perlu dikerjakan sebagai cluster/versi tersendiri sebelum v0.13.4 bisa lanjut — decision-gate scope lengkap terpisah saat sprint ini benar-benar dimulai (BOSS-003) — jangan asumsikan detail apa pun dari baris ini | **In Progress** — **v0.26.1 Selesai** (merged `develop`→`main`, tagged `v0.26.1`, 2026-09-15): `work_orders.scheduled_at` (janji hari+jam kunjungan, sudah ada di skema sejak v0.5.0 tapi dead) genuinely bisa diisi lewat `StoreWorkOrderRequest` (`POST /api/v1/work-orders`) dan diedit pasca-create lewat UI "Atur Jadwal" di `WorkOrderShow` (`WorkOrderService::scheduleVisit()`) — nol logic dispatch/notifikasi WA di sini, murni wiring. **v0.26.2 Selesai** (merged `develop`→`main`, tagged `v0.26.2`, 2026-09-16 — 3 sub-versi menumpuk di 1 branch sebelum sempat di-tag terpisah, cuma 1 tag di titik merge ini, bukan 3): Settings "Konfig WA Gateway" (`work_order_dispatch_settings` per-tenant, dispatch_offset_minutes/reminder window) + command `work-orders:dispatch` (self-throttle per-tenant, `->everyMinute()`) + hook HYBRID di `WorkOrderService::createFromSubscription()` (WO tanpa janji dispatch segera in-line, WO dengan janji menunggu command) + halaman list `/work-orders` (sidebar "Pelanggan", bukan CRUD penuh — filter status, assign teknisi inline reuse `assignTechnician()`/`WorkOrderPolicy::manage()`). **Approach registrasi FINAL: satu pintu** — investigasi menemukan alur registrasi→subscription→WO sebelumnya TIDAK PERNAH otomatis sama sekali (2 form manual independen, bukan async); pendekatan awal (field "Janji Kunjungan" di form "Buat Langganan" terpisah, sempat dibangun sebagai langkah antara) DIGANTIKAN — field ppp_package_id di form "Registrasi Pelanggan" sekarang WAJIB (dulu opsional/dead sejak v0.9.4) dan submit-nya langsung membuat Customer+Subscription+WorkOrder dalam SATU transaksi DB (`RegistrationService::register()`, parameter `$subscription` opsional — backward-compatible untuk API). Field Janji Kunjungan di form "Buat Langganan" TETAP ADA (dipakai pelanggan existing nambah langganan baru), tapi menu "Subscriptions" di-unlink dari sidebar; tombol "+ Pelanggan Baru" (CreateCustomerAction inline, duplikat fungsi registrasi tanpa NIK/koordinat/referrer/paket/komisi) dihapus dari `/customers`. **v0.26.3 Selesai** (merged `develop`→`main`, tagged `v0.26.3`, 2026-09-16 — merge diinstruksikan langsung Agung tanpa verifikasi manual browser terpisah, beda dari pola closure sebelumnya di cluster ini, dicatat eksplisit di commit merge-nya): `WhatsappEventType::WorkOrderDispatched` (baru) — SATU event type mencakup dispatch awal DAN reminder H+ lewat variabel `{status_notice}` (`WhatsappTemplateService::resolve()` cuma resolve 1 template per event_type, bukan 2 template terpisah). Temuan Langkah 0 mengubah desain: di titik dispatch, `technician_id` HAMPIR SELALU masih null secara struktural (`assignTechnician()` cuma valid dari status `Ready`, jauh setelah dispatch terjadi) — jadi WA japri di titik DISPATCH AWAL BROADCAST ke semua teknisi aktif tenant terkait ("siapa cepat dia dapat"), bukan ke 1 teknisi. Untuk REMINDER: kalau `technician_id` sudah terisi (assign manual sudah terjadi), kirim CUMA ke teknisi itu; kalau belum, broadcast lagi. Kirim ke grup WA TETAP placeholder — belum dikerjakan. **v0.26.4 Selesai** (merged `develop`→`main`, tagged `v0.26.0` di commit merge `766aeb0` 2026-09-17 — satu tag penutup untuk SELURUH cluster, bukan tag per sub-versi individual, karena semua sub-versi menumpuk sampai ke titik merge ini): kapabilitas kirim ke grup WA di gateway Go/whatsmeow — `GET /sessions/{key}/groups` (`Manager.ListGroups()` → `client.GetJoinedGroups()`) + `POST /sessions/{key}/send-group` (`Manager.SendGroupMessage()`, guard `types.ParseJID()` + `Server=="g.us"`, kegagalan permanen — bot bukan anggota grup lagi — ditandai `IsPermanentGroupSendError()` dikonfirmasi lewat pembacaan langsung source whatsmeow); Laravel `WhatsappSessionService::listGroups()` + `WhatsappGatewayService::buildAndQueueForGroup()` (Opsi A: `phone_number` dipakai ulang untuk JID grup mentah, tanpa kolom baru) + `SendWhatsappGroupMessageJob` (retry/backoff identik job individu, TAPI flag `permanent` dari gateway → langsung `failed` tanpa retry berulang); dropdown pilih grup (baca live dari gateway, bukan input JID manual) di Settings "Konfig WA Gateway". Full regression suite gerbang penutup cluster: 2175 passed, 0 failed, 6684 assertions. **Verifikasi manual 2026-09-17**: Lapis 1 (kirim satu pesan test manual ke grup asli "Token Wifi - Support - Bajastu", `dispatchSync()` langsung tanpa lewat siklus dispatch otomatis) **SUKSES** — dikonfirmasi masuk ke HP Agung. Lapis 2 (simulasi siklus dispatch penuh dengan broadcast japri ke teknisi asli) **TIDAK bisa lanjut** — investigasi menemukan gap arsitektur nyata (tabel `technicians` tidak tersinkron dengan staff role teknisi, lihat entry Backlog baru di bawah) sehingga tidak ada satu pun teknisi ASLI yang bisa dites; ditutup sebagai gap Backlog terpisah, BUKAN blocker closure cluster ini — kapabilitas kirim grup WA sendiri (fokus utama v0.26.4) sudah terverifikasi bekerja penuh. Sebelum closure, 2 baris `Technician` dummy ("TEST - Jangan Hapus") + 1 `WhatsappMessageLog` test dibersihkan dari DB (backup diambil lebih dulu) — **`Technician::count()` = 0 pasca-closure, lihat catatan status di bawah.** Hotfix `disable-unrecognized-fallback` (toggle mematikan sementara auto-reply `UnrecognizedMessageFallback` untuk keperluan pairing ulang sesi WA "direct", lihat section terpisah di bawah) ikut merge di titik yang sama — toggle TETAP dimatikan pasca-closure, menghidupkannya lagi adalah keputusan terpisah Agung. **Gap `technicians` DITUTUP 2026-09-20**: 3 baris `Technician` dibuat manual untuk staff teknisi asli (firman/Yusuf/Sahrul, `user_id`/`tenant_id`/`phone` sesuai akun `/staff` masing-masing) — QUICK-FIX MANUAL untuk unblock testing, BUKAN mekanisme sinkronisasi otomatis permanen (itu tetap Backlog terpisah, opsi arsitektur — sinkron otomatis saat role teknisi diberikan, atau evaluasi ulang perlu-tidaknya tabel `technicians` terpisah — belum diputuskan). **`v0.26.0.1` Selesai** (2026-09-20, tag `v0.26.0.1` — patch kecil di atas tag `v0.26.0`, branch `v0.26.0-followup-wo-template-fields` dari `main`): 2 variabel baru di `WorkOrderDispatchService::messageVariablesFor()` — `customer_cid` (dari `customers.cid`, kolom TERPISAH dari `id`/`nik`, dikonfirmasi Langkah 0 sebelum dipakai — bukan NIK seperti dugaan awal) dan `customer_coordinates` (link Google Maps "directions", format PERSIS sama dengan yang sudah dipakai `OdpEdit`/`FiberNodeDetail`/`FiberNodeIndex` — `https://www.google.com/maps/dir/?api=1&destination={lat},{long}`, `'-'` kalau lat/long `null`, kondisi NORMAL saat ini karena SEMUA customer di database belum punya koordinat terisi, v0.16.0 Langkah 12). `WhatsappMessageTemplate` `WorkOrderDispatched` tenant_id=1 diisi versi produksi. **Verifikasi manual 2026-09-20 — LENGKAP untuk Lapis 1 & 2**: Lapis 1 (kirim manual ke grup) SUKSES; **Lapis 2 (simulasi dispatch penuh, KALI INI dengan 3 teknisi ASLI + grup sekaligus, bukan dummy)** — WO test → `dispatched_at` terisi, 4/4 `WhatsappMessageLog` **`status=sent`** (Sahrul/Yusuf/firman/grup Token Wifi - Support - Bajastu), dikonfirmasi Agung diterima dengan benar di semua 4 tujuan. Data test (Customer/Subscription/WorkOrder/4 log) dibersihkan setelah verifikasi; 3 baris `Technician` DIPERTAHANKAN permanen. **⚠️ Lapis 3 (reminder H+1) BELUM diverifikasi manual — SENGAJA DITUNDA** (tidak disimulasikan via `Carbon::setTestNow()`) — akan tervalidasi NATURAL besok pagi saat command `work-orders:dispatch` genuinely jalan jam 08:00 pertama kali setelah 2026-09-20. **Cek `WhatsappMessageLog` (event_type=`work_order_dispatched`, `status`) besok pagi untuk konfirmasi** — kalau ada masalah (0 log baru, status `failed`, atau isi pesan salah), balik ke chat/topik "v0.26.0 Work Order" untuk investigasi lanjutan, jangan mulai dari nol. |

## ⚠️ HOTFIX SEMENTARA — UnrecognizedMessageFallback DIMATIKAN (2026-09-16, cek sebelum mulai v0.13.4)

**⚠️ 2026-09-16: UnrecognizedMessageFallback auto-reply DIMATIKAN SEMENTARA (lewat config
`services.whatsapp_gateway.unrecognized_fallback_enabled`, env `WHATSAPP_UNRECOGNIZED_FALLBACK_ENABLED=false`
di root `.env` server ini) untuk keperluan pairing ulang sesi WA "direct". CEK & PUTUSKAN ulang status ini
(nyalakan lagi atau tetap mati) sebelum mulai coding v0.13.4.**

**Update 2026-09-17**: kode toggle ini sudah **merge ke `main`** sebagai bagian closure cluster v0.26.0
(di-cherry-pick dari branch `hotfix/disable-unrecognized-fallback` ke `v0.26.4-wa-group-dispatch`, lalu
ikut merge `--no-ff` di titik yang sama dengan v0.26.4, commit `766aeb0`, tag `v0.26.0`) — bukan lagi
tinggal di branch terpisah. **Toggle di server ini TETAP `false`/mati pasca-closure** — closure cluster
v0.26.0 TIDAK menyalakannya kembali secara otomatis, itu keputusan terpisah Agung, kapan pun siap pairing
produksi sesi "direct" beneran (mekanisme PERSIS untuk menyalakan lagi, TIDAK berubah, ada di bawah).

**Kenapa ada toggle ini** — `App\Services\Whatsapp\WhatsappIncomingMessageService::
routeToConversationOrFallback()` (v0.13.3, lihat baris `v0.13.0` di tabel atas) mem-balas OTOMATIS SIAPA
PUN yang kirim pesan ke sesi WA "direct" tanpa match state percakapan aktif (v0.13.2) atau teknisi
terautorisasi (v0.13.3) — `WhatsappEventType::UnrecognizedMessageFallback`, dipicu SINKRON dari
`recordFromWebhook()` (handler `POST /api/v1/whatsapp/webhook/incoming-message`, jalan inline di request
HTTP dari `whatsapp-gateway`, BUKAN via queue). Ini mengganggu proses pairing ulang (Agung perlu re-scan QR
sesi "direct" setelah sengaja logout — setiap pesan test/percobaan pairing otomatis dibalas auto-reply,
mengacaukan proses). Fitur ini SENGAJA TIDAK DIHAPUS (masih dipakai untuk fallback generik pesan tak
dikenal ke pelanggan biasa) — cuma dimatikan SEMENTARA lewat toggle, feature-nya sendiri utuh.

**Mekanisme PERSIS untuk menyalakan lagi**:
1. Set `WHATSAPP_UNRECOGNIZED_FALLBACK_ENABLED=false` → `true` (atau hapus baris-nya — default value di
   `config/services.php` adalah `true`) di root **`.env`** server ini (`/opt/boss-app/.env` — BUKAN
   `.env.example`, BUKAN `app/.env`).
2. **Recreate** (bukan restart) container `boss-app`:
   `docker compose up -d boss-app` — env var baru tidak terbaca kalau cuma `docker compose restart
   boss-app` (gotcha env-var-container yang sudah berulang kali dicatat di CLAUDE.md). Tidak perlu
   recreate container lain — handler ini jalan sinkron di dalam request `boss-app` sendiri, bukan lewat
   queue worker.
3. Default value kalau env var tidak di-set sama sekali = `true` (perilaku sebelum hotfix ini, fallback
   auto-reply AKTIF) — jadi kalau ragu, cukup HAPUS baris `WHATSAPP_UNRECOGNIZED_FALLBACK_ENABLED` dari
   `.env` + recreate `boss-app`.
4. Kode toggle-nya ada di `App\Services\Whatsapp\WhatsappIncomingMessageService::
   routeToConversationOrFallback()` (guard `config('services.whatsapp_gateway.unrecognized_fallback_enabled')`,
   early-return + log kalau `false`) dan deklarasi config-nya di `config/services.php` (key
   `whatsapp_gateway.unrecognized_fallback_enabled`).

**Status**: sudah di `main` (lihat "Update 2026-09-17" di atas) — bukan lagi menunggu merge/tag terpisah.
Scoped test `WhatsappIncomingMessageWebhookTest` (19/19 hijau, termasuk kasus fallback generik tetap lolos
karena default config `true`) + Pint bersih, dikonfirmasi ulang sebelum closure cluster v0.26.0. Branch
asli `hotfix/disable-unrecognized-fallback` dipertahankan di repo (kebiasaan proyek, tidak dihapus) tapi
riwayatnya sudah sepenuhnya masuk `main` lewat cherry-pick + merge — jangan bingung kalau branch itu masih
terlihat ada, itu bukan berarti belum closed.

## Perbaikan Pasca-Rilis v0.13.1

**BUKAN sub-versi/tag baru** — koreksi bug atas fitur yang sudah di-tag `v0.13.1` (listener pesan masuk
WhatsApp), bukan fitur baru. Detail lengkap ada di histori commit (`fix(whatsapp): bug LID...`,
`fix(whatsapp): cegah 1 nomor WA...`, `feat(whatsapp): tombol Logout...`) — ringkasan singkat di sini.

- **Bug LID** — `sender_phone` di `whatsapp_incoming_messages` kadang tersimpan sebagai raw WhatsApp LID
  (Linked ID) mentah, bukan nomor telepon asli. Fix: deteksi via `Sender.Server` (bukan `AddressingMode`,
  dikonfirmasi field itu kadang kosong), urutan fallback (SenderAlt dari event → lookup LIDStore lokal →
  kolom `is_lid` + placeholder "Nomor tidak tersedia" di UI kalau semua gagal), plus retroactive backfill
  (begitu PN sebuah kontak LID ketahuan, semua baris lama kontak yang sama ikut diperbarui).
- **Bug `ResellerContext` tidak persist antar request Livewire** (render+tab visibility, dan 4 action
  methods) — **catatan**: ini genuinely ditemukan+diperbaiki SEBELUM tag `v0.13.1` dibuat, sebagai bagian
  closure v0.13.1 itu sendiri (bukan pasca-rilis sesungguhnya) — disebut di sini murni untuk kelengkapan
  ringkasan, bukan patch terpisah.
- **Bug cegah-dobel-session** — migration `status_reason` sempat pending sementara kode sudah menulis ke
  kolom itu → setiap UPDATE status Connected/RejectedDuplicate gagal diam-diam (SQL error tertangkap
  try/catch generik, tetap balas 200 ke gateway) → status BOSS App nyangkut di "Menunggu Scan QR" walau WA
  sudah connect di HP. Migration sudah dijalankan, root cause tertutup. Fitur cegah-dobel-session sendiri
  (1 nomor WA = 1 session, reject + logout paksa + notifikasi WA) baru dibangun di patch ini.
...
Seluruh scope Commission selesai + merged + tagged: v0.9.1 (rename Agent→Referrer), v0.9.2 (CRUD
Referrer + Portal + RBAC two-tier), v0.9.3 (Commission Rate Settings), v0.9.4 (skema komisi per
pelanggan), v0.9.5 (auto-maturity per invoice lunas — **Eligibility**), v0.9.6 (Fitur Titip + OTP),
v0.9.7 (Login Terpadu), v0.9.8 (Perpanjang di Daftar Pelanggan + Fee Komisi + Multi-Bulan), v0.9.9
(WhatsApp Gateway Reliability), v0.9.10 (migrasi whatsmeow), v0.9.11 (**Payment** — payout Titip instan
+ batch bulanan per jendela rate), v0.9.12 (Perpanjang → Invoice ASLI + cetak), **v0.9.13 (Approval +
Clawback — menutup slot scope "v0.9.0")**. Tidak ada lagi sisa scope Commission di roadmap.
...
2. **`v0.15.0` dipakai untuk reservasi slot "Management Ticketing"** — slot
   ...
   bagian di bawah). **`v0.9.3` (Commission Rate Settings) + `v0.9.4`
   (Skema Komisi per Pelanggan) SUDAH SELESAI & di-tag `v0.9.4`** — lihat
   baris tabel. Rentang `v0.15.0`-`v0.20.0` dikonfirmasi genuinely kosong
   total (nol referensi di seluruh file) sebelum amandemen ini.
3. **`v0.18.0`/`v0.19.0` didaftarkan (2026-09-02)** untuk 2 slot kosong
...
4. **`v0.20.0`/`v0.21.0` didaftarkan (2026-09-02)** untuk 2 slot kosong
   berikutnya — `v0.20.0` "Template Halaman Isolir (Captive Page
   Non-Aktif)" dan `v0.21.0` "Template Voucher Cetak (Token/Hotspot)"
   (lihat baris tabel masing-masing). Keduanya murni reservasi nomor+nama,
   scope detail belum ditentukan — masing-masing punya decision-gate
   terpisah.
5. **`v0.22.0` "Manajemen User (Staff + Referral Terpadu)" (2026-09-02)** —
   awalnya didaftarkan di branch `manajemen-user` (dari `main`), TAPI branch
   itu ditinggalkan (stale, nol kode genuinely dibangun) — dikerjakan ulang
   dari `main` di branch `v0.22.1-staff-crud`. Baris tabelnya baru
   ditambahkan saat v0.22.1 ditutup (2026-09-15), lihat baris tabel sendiri.
6. **`v0.23.0` "OMCI Integration untuk Zero-Touch ONT Provisioning"
   (2026-09-04)** — lihat baris tabel. Reservasi nomor+nama, decision-gate
   terpisah (vendor OLT prioritas, DHCP Option 43 vs OMCI penuh).
   `v0.24.0` genuinely kosong.
7. **`v0.26.0` "WO Auto-Dispatch Scheduling + Notifikasi (Prasyarat v0.13.4)"
   (2026-09-15)** — dicek dulu sebelum menulis: `v0.24.0` (Redundansi
   WireGuard Node) dan `v0.25.0` (Arsitektur Multi-Tenant SaaS) sudah
   punya baris tabel sendiri (ditambahkan setelah log item 6 di atas
   ditulis, tanpa log-nya sendiri diupdate — bukan sesuatu yang perlu
   diperbaiki di sini, cuma dicatat). `v0.26.0` adalah slot kosong
   pertama yang genuinely tersedia setelah itu (dikonfirmasi lewat cek
   seluruh file, bukan diasumsikan). **BEDA dari reservasi nomor+nama
   murni lainnya di atas** — slot ini punya scope AWAL yang sudah
   konkret (lihat baris tabel) hasil investigasi read-only 2026-09-15,
   bukan sekadar nama placeholder kosong, karena posisinya sebagai
   prasyarat blocking `v0.13.4`.
...
**CLUSTER SELESAI & DI-TAG (2026-09-01).** Seluruh v0.14.1–v0.14.7 sudah merged ke `develop`/`main` dan
di-tag. Tag yang ada: `v0.14.1`, `v0.14.2` (mencakup revisi v0.14.2.1 RouterOS Live-Push start +
...
- **v0.14.7 Rollout Produksi — Verifikasi Kesiapan `test-x86-bajastu`** — BUKAN migrasi massal. Verifikasi
  terkontrol (Langkah 0 inventaris namespace + Langkah 1 uji tulis-lalu-hapus) bahwa NAS id=1 aman jadi
  target Profil Paket untuk pelanggan BARU. Uji lolos 2026-09-01: 0 gangguan ke 218 sesi aktif, 0 object
  existing berubah, router bersih 100% setelah cleanup. Detail: `docs/test-x86-bajastu-namespace-inventory.md`
  + CLAUDE.md bagian "v0.14.7 — test-x86-bajastu RESMI SIAP...". **295 pelanggan existing tetap di sistem
  lama, tidak dimigrasikan.**
...
## v0.14.5 — Profil PPP (branch `v0.14.5-profil-ppp`, merged + tagged `v0.14.5`)

Sub-versi kelima cluster "Profil Paket" — paket bulanan PPPoE, setara Profil Hotspot (v0.14.4) tapi untuk
pelanggan PPP. Branch dibuat dari `main` di titik tag `v0.14.4.1` (dikonfirmasi langsung lewat `git log`/
`git tag`, bukan diasumsikan) — branch `v0.14.5-profil-ppp` yang sebelumnya sempat dibuat untuk investigasi
Langkah 0 saja (nol commit di luar `main` lama) dihapus dan dibuat ulang dari titik terkini, karena sudah
basi 3 commit di belakang `main` (mendahului `v0.14.4.1`).

**Arsitektur (dikonfirmasi sebelum kode ditulis)**: setiap Profil PPP push `/ppp profile` BARU/TERPISAH ke
router — bukan numpuk ke `/ppp profile` milik Grup Profil induknya. `local-address`/`dns-server`/
`parent-queue` DIWARISI dari Grup Profil dan di-resolve LIVE setiap push (bukan disalin sekali — reuse pola
yang sama dengan Profil Hotspot mewarisi `address-pool`); `rate-limit` dari Bandwidth Profile sendiri,
`session-timeout` dari Masa Aktif sendiri (SELALU ada — beda dari Profil Hotspot yang opsional lewat toggle
Unlimited/Limited, paket bulanan PPP selalu punya durasi nyata). `/ppp profile` MENDUKUNG `comment`
(dikonfirmasi live) — dipakai pola lookup-by-comment yang sudah teruji di CustomerIpPool/NetworkProfileGroup,
BUKAN workaround `mikrotik_profile_name` yang terpaksa dipakai HotspotPackage (itu cuma perlu karena
keterbatasan spesifik `/ip hotspot user profile` yang menolak `comment`, tidak berlaku di sini).

**Risiko collision nama — inti alasan sub-versi ini punya lapisan validasi ekstra**: nama `/ppp profile`
yang di-generate Profil PPP WAJIB dijamin tidak bentrok dengan nama `/ppp profile` milik Grup Profil MANAPUN
di NAS yang sama (keduanya push ke namespace `/ppp profile` yang sama di router meski beda tabel di BOSS
App, dan namespace itu di-scope per-NAS bukan per-Grup-Profil). `PppPackage::collidesWithExistingName()`
(method statis pada model, query lintas `network_profile_groups` DAN `ppp_packages` di-scope `nas_id`,
mengecualikan baris sendiri saat edit) dipanggil identik dari `Store`/`UpdatePppPackageRequest::
withValidator()` DAN `PppPackageIndex` (Livewire) — sama pola "validasi cross-entity di layer validasi, bukan
di Service" yang sudah mapan di codebase ini.

**`RouterOsGateway::syncPppProfile()` diperluas lagi** — 2 parameter trailing baru, `$rateLimit`/
`$sessionTimeout` (default `null`), dikonfirmasi live terhadap `ro-hotspot.bajastu.id`: `rate-limit`
menerima format `"{kbps}k/{kbps}k"` yang sama persis dengan `/ip hotspot user profile` (v0.14.4);
`session-timeout` menerima suffix waktu m/h/d yang sama persis (RouterOS menormalisasi tampilan readback —
mis. `"30d"` yang dikirim terbaca kembali sebagai `"4w2d"`, durasi sebenarnya tidak berubah). `HotspotDurationUnit`
(enum, `App\Enums`) DIPAKAI ULANG untuk `active_duration_unit` Profil PPP — bukan `PppDurationUnit` baru —
karena logika konversinya (suffix m/h/d, tanpa bulan native, aproksimasi 30 hari/bulan) identik dan sudah
diverifikasi empiris sekali di v0.14.4, dikonfirmasi ulang berlaku sama persis di `/ppp profile` lewat live
test segar sub-versi ini. Perluasan signature ini mengharuskan pembaruan mekanis di 13 file fake
`RouterOsGateway` di test suite (interface + parameter opsional baru, PHP mewajibkan signature cocok) —
regresi penuh dijalankan ulang setelahnya untuk memastikan tidak ada yang rusak sebelum lanjut menulis kode
baru.

**Tabel `ppp_packages`** — TIDAK ada `profile_type`/`limit_type`/`quota_*`/`show_in_voucher_form` (konsep
Unlimited/Limited/TimeBase/QuotaBase murni milik Hotspot, paket bulanan PPP tidak pakai ini sama sekali) —
`active_duration_value`/`active_duration_unit` NOT NULL (selalu wajib, bukan nullable seperti Hotspot).
Unique index `(network_profile_group_id, name)` cuma baseline per-parent (pola sama HotspotPackage) — bukan
penjaga collision sesungguhnya, itu tugas `collidesWithExistingName()` di layer validasi (lihat di atas).

**Diverifikasi manual REAL, end-to-end, terhadap `ro-hotspot.bajastu.id` saja, semua dieksekusi langsung**:
- **Push**: Profil PPP nyata mereferensikan Grup Profil `#11` ("test-10Mbps-HomeFixed") + Bandwidth Profile
  "10Mbps" (15000k/15000k) + Masa Aktif 1 bulan — dikonfirmasi muncul sebagai `/ppp profile` BARU
  (`BOSS-TEST-PPPPKG-DELETE-ME`, comment `BOSS App - PPP Package #1`) yang GENUINELY TERPISAH dari
  `/ppp profile` Grup Profil induknya sendiri (`test-10Mbps-HomeFixed`, comment `BOSS App - Network Profile
  Group #11` — keduanya hidup berdampingan di router, tidak saling menimpa). Field lengkap benar:
  `remote-address=Hotspot-10Mbps` (pool warisan Grup Profil), `dns-server=1.1.1.1` (warisan Grup Profil),
  `rate-limit=15000k/15000k` (dari Bandwidth Profile), `session-timeout=4w2d` (30 hari, dari Masa Aktif 1
  bulan).
- **Edit**: ubah Masa Aktif ke 2 bulan, push ulang — `.id` router yang SAMA ter-update di tempat (dikonfirmasi
  cuma 1 objek dengan comment itu, bukan duplikat), `session-timeout` berubah jadi `8w4d` (60 hari).
- **Delete**: dikonfirmasi genuinely hilang dari `/ppp profile print` — router kembali ke 5 entry aslinya.
- **Validasi tipe**: Grup Profil bertipe Hotspot ditolak (Livewire + API), baik saat create maupun update.
- **Validasi collision**: nama yang sama dengan nama Grup Profil sendiri ditolak; nama yang sama dengan
  Profil PPP lain di Grup Profil BERBEDA tapi NAS SAMA ditolak; nama yang sama tapi di NAS BERBEDA tetap
  diizinkan (dikonfirmasi eksplisit, bukan cuma diasumsikan aman).
- Sidebar "Profil PPP" (child ke-3 di bawah "Profil Paket") dan halaman `/ppp-packages` dikonfirmasi
  ter-render nyata lewat request HTTP sungguhan (login session + panggilan endpoint langsung ke
  `boss-nginx`), bukan cuma dibaca dari kode.

`test-x86-bajastu` tidak disentuh sama sekali. Semua artifact test (`PppPackage` + objek router-nya)
dibersihkan lewat jalur delete asli setelahnya; router kembali ke state pristine (5 `/ppp profile`, sama
seperti sebelum sub-versi ini dimulai).

**Regresi**: 36 test baru (`PppPackageMikrotikSyncTest` 9 test Job/RouterOS sync,
`PppPackageIndexLivewireTest` 9 test Livewire, `PppPackageApiTest` 11 test REST API, `PppPackageTest` 8 test
unit model termasuk `collidesWithExistingName()`), full regression suite dijalankan ulang, Pint clean di
semua file yang disentuh. Permission `ppp_packages.view`/`.manage` di-seed ulang ke database dev real
(disiplin yang sama dari insiden-insiden sebelumnya di cluster ini — jangan asumsikan migration/kode saja
cukup).
...
**Status: Selesai — merged `--no-ff` ke `develop` lalu `develop`→`main` (pola v0.16.0/v0.17.0), tag annotated
`v0.16.1` di commit merge `develop`→`main`. Verifikasi manual browser oleh Agung sepanjang sesi (4 putaran
revisi).** DB dev sudah di-`migrate` (migration `fiber_core_splices` + `pon_port_count` dari revisi di
bawah) — lebih maju dari `main`, closure harus segera menyusul (pelajaran v0.14.5.1).

### Revisi (review Agung, sama branch, sebelum closure)

**A — Undo cepat di Mode Edit Rute.** Tombol "Undo Titik Terakhir" di toolbar edit — sekali klik hapus
waypoint yang paling baru ditambahkan, bisa diklik berkali-kali. Tetap client-side sampai "Simpan Rute".
(`fiberTopologyMap.undoLastWaypoint()`.)

**B — Label tube+warna penuh di dropdown Assign Core-to-Core.** "T1/C1 (Biru)" → "Tube 1 (Biru) / Core 1
(Biru)" di kedua dropdown; warna tube DAN core dari nama tersimpan (bisa beda kalau core di-override).

**C — Guard arah splice + hapus kabel.** `FiberCoreSpliceService::createSplice()` guard baru: splice WAJIB
menghubungkan kabel MASUK (`fiber_cable.to_* == node`) dengan kabel KELUAR (`fiber_cable.from_* == node`)
— tolak masuk+masuk / keluar+keluar. TIDAK ada batasan jumlah kabel per arah (boleh 2 masuk vs 1 keluar,
dst) — guard cuma soal arah per-pasangan. **Hapus kabel**: tombol "Hapus" per kartu kabel di "Diagram
Splice" (masuk & keluar), `wire:confirm` menjelaskan konsekuensi. Cascade DB sudah lengkap (`fiber_cores`
→ `fiber_core_port_logs` / `fiber_core_splices` / `fiber_accessories`; `fiber_cable_waypoints` langsung) —
splice aktif ikut terhapus via cascade, tidak diblokir. Titik anak di ujung kabel keluar tidak ikut.

**D — ODP di form "Titik Baru" — SELESAI (10 keputusan dikonfirmasi Agung).** "ODP" jadi opsi Tipe Titik
di `FiberNodeForm` **hanya saat create** (edit ODP tetap lewat `OdpEdit`). Field kondisional saat
Tipe=ODP: "Kode ODP" (wajib, unik per tenant), "Nama ODP" (wajib), lat/long WAJIB (kolom `odps` NOT
NULL), "Jumlah Port ODP" (→ `odps.total_ports`, field OTB `port_count` disembunyikan), loss in/out
**WAJIB** (konsisten `OdpEdit` + keputusan arsitektur v0.16.0 L3 "WAJIB untuk Odp"), splitter form muncul
juga. Simpan → `FiberTopologyService::createOdpWithAttachments()` (baru): `Odp::create()` +
`provisionPorts()` + foto + splitter dalam 1 transaction, `reseller_id = null`, **tidak menyentuh
`StoreOdpRequest`/`OdpController`** (pola `OdpEdit`/`updateOdpTopologyFields`). Gate tetap
`network_infrastructure.manage` (dikonfirmasi lolos untuk tier-admin, sama seperti `OdpPolicy::create`).

**E — Cari + tukar cepat di "Assign Port ke Core" (OTB).** Field cari (nomor port / nama tube / warna
core / kabel — filter di `render()`). Aksi "Tukar Port": pilih 2 baris core (checkbox) →
`FiberTopologyService::swapCorePorts()` menukar `port_number` dua core dalam satu transaction (aman tanpa
langkah kosongkan-dulu — `fiber_cores.port_number` tidak punya unique DB, cuma app-enforced). Tiap core
tetap pegang OLT/PON-nya sendiri, cuma `port_number` yang ditukar. Gate form pakai flag "ada core
assignable" (bukan hasil filter) supaya kotak cari tidak ikut hilang saat 0 hasil.

**F — OLT/PON dropdown terstruktur (tanpa monitoring live).** Kolom baru `olt_devices.pon_port_count`
(unsignedInteger nullable), diisi MANUAL admin di form OLT ("Jumlah PON Port (opsional)"). Di form
assign-port: OLT yang `pon_port_count`-nya terisi → `olt_pon_port_label` jadi dropdown "PON 1".."PON N";
yang null tetap teks bebas (tidak dipaksa). Data lama tidak dimigrasikan paksa.
...
**v0.16.0 Core Network Infrastructure Management — SELESAI & DI-TAG.** Langkah 0-13 (cleanup 9B dieksekusi;
container `osrm` verified; draft offline diverifikasi masih jalan). Langkah 0-4 di commit
`396b6ba`/`701445f`/`1b82c61`/`886d02c`; Langkah 5-13 di satu commit `29d51a2` (branch
...
**Akar masalah**: tabel `technicians` (migration v0.5.0, 2026-08-03 — punya kolom `user_id` FK ke `users`
sejak awal, jadi relasi STRUKTURAL memang ada) dan sistem role Spatie `"teknisi"` di `users` (v0.22
Manajemen Staff, migration-migration 2026-08-25) adalah **dua sistem yang sejak awal tidak pernah
diproses-sambungkan** — tidak ada satu titik pun (create staff via `/staff`, assign role, atau lainnya)
yang otomatis membuat baris `technicians` untuk staff baru berrole teknisi. Satu-satunya jalur pembuatan
baris `technicians` adalah `POST /technicians` (`TechnicianController::store()`, REST API murni sejak
v0.5.0, tidak ada UI Livewire sama sekali) — `user_id` wajib diinput manual, TANPA validasi bahwa user itu
punya role Spatie "teknisi". `StaffService::delete()` (v0.22) TAHU tentang tabel `technicians` (guard
mencegah hapus staff yang masih terdaftar sebagai Technician aktif) tapi tidak pernah membuat barisnya.

**Konsekuensi nyata, dikonfirmasi lewat investigasi langsung**: sebelum cleanup closure v0.26.0, 3 staff
role teknisi asli (Firman, Sahrul, Yusuf — nomor HP asli) yang muncul Enabled di `/staff` **genuinely 0
baris di `technicians`**; satu-satunya 2 baris `technicians` yang ada di seluruh database (sebelum
dibersihkan) adalah dummy lama ("TEST - Jangan Hapus", nomor `080000000001`/`080000000002`) dari testing
v0.5.0. `WorkOrderDispatchService::notifyTechnicians()` (broadcast japri WA WO, v0.26.3/v0.26.4) dan
dropdown assign teknisi di `/work-orders` sama sekali tidak "melihat" Firman/Sahrul/Yusuf.

**Status pasca-cleanup closure v0.26.0**: 2 baris `Technician` dummy sudah dihapus (bersama
`WorkOrder #4`'s `technician_id` di-null-kan dulu, dan `WhatsappMessageLog` test terkait) — **`Technician::count()` = 0 di seluruh database saat ini.** Konsekuensi langsung: **broadcast japri individu WO
(`WhatsappEventType::WorkOrderDispatched` ke teknisi, v0.26.3) tidak akan terkirim ke siapa pun sampai gap
ini diselesaikan** (query `Technician::active()` akan selalu kosong). **Notifikasi ke GRUP WA (v0.26.4)
tetap berfungsi normal** — tidak bergantung pada tabel `technicians` sama sekali, sudah terverifikasi
sukses terkirim ke grup asli "Token Wifi - Support - Bajastu".

**Perlu keputusan arsitektur Agung, belum diputuskan**: (a) auto-create/link baris `technicians` otomatis
saat role Spatie "teknisi" diberikan ke seorang staff (di `StaffService::create()`/`update()` atau
sejenisnya), atau (b) evaluasi ulang apakah tabel `technicians` yang terpisah masih diperlukan sama sekali
— mengingat sudah ada `users`+role Spatie sebagai sistem identitas staff yang lebih umum sejak v0.22, atau
