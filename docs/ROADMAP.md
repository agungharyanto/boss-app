...
| v0.14.5.4 | Network       | RouterOS Live-Push — `local-address` dari `gateway_ip` + Ensure IP Pool Sebelum Push Profil | Dua bug jalur push `/ppp profile` (Grup Profil ppp + Profil PPP + Profil Expired), ditemukan lewat investigasi read-only `ro-hotspot.bajastu.id` setelah drift manual (Agung hapus semua `/ip pool` di WinBox). **BUG #1**: `customer_ip_pools.gateway_ip` (disimpan sejak v0.14.2 khusus untuk `/ppp profile local-address`) tidak pernah dikirim — wiring kelewat di v0.14.3/v0.14.5 → PPP klien connect tapi tak dapat routing. **BUG #2**: push `/ppp profile` yang mereferensikan `/ip pool` (`remote-address=<nama pool>`) TANPA pastikan pool ada dulu → RouterOS tolak "invalid value for argument remote-address:" → resync gagal PERMANEN setelah admin hapus pool manual. Fix: `syncIpPool()` (idempoten by-comment) dipanggil DULU sebelum `syncPppProfile()` di ketiga Job; `$pool->gateway_ip` di-resolve live sebagai `$localAddress`. Diverifikasi nyata: Grup Profil #26 `PPPoE-Remote` dari `failed` → 1× "Sync Ulang" → `synced`, pool + profile + pppoe-server dibuat ulang otomatis, `local-address=10.0.1.1`. `test-x86-bajastu` tidak disentuh. | Selesai — branch `investigasi-local-address-drift` |
