...
| v0.9.6  | Billing & Finance | Fitur Titip (Self-Service + OTP WhatsApp) | Referrer mencatat sendiri pembayaran cash "titip" dari pelanggan yang ia referensikan lewat Portal Referrer → dapat komisi Titip (`CommissionRate.titip_amount`). HANYA untuk pelanggan yang punya `ppp_package_id` + rate Titip valid. Alur: layar konfirmasi detail → OTP 6-digit ke WhatsApp **Referrer sendiri** (event type WhatsApp ke-5 `referrer_action_otp`, penerima = `referrers.phone`) → verifikasi → baris `commission_ledger` langsung `scheme=titip status=eligible` (OTP = jaring pengaman, nominal dikunci ke rate — tidak ada approval admin, tidak ada input manual). `commission_ledger.payment_period` (date) untuk guard duplikat per-bulan (peringatan, bukan hard block). CREATE-ONLY (Referrer tidak bisa edit/hapus). Dashboard admin "Titip Masuk" (`/titip-masuk`, `commission_ledger.view`) = daftar kerja perpanjangan manual, BUKAN gate approval. **TIDAK ada otomasi NAS/RADIUS/MixRadius** — perpanjangan tetap manual admin. Plus fondasi: `CommissionScheme::Titip`, `ReferrerReferralResource` tampilkan SEMUA baris komisi (bukan `->first()`), Rekap Komisi portal diisi. | Implementasi selesai, belum di-merge/tag — menunggu verifikasi manual Agung |
