...
di root `.env` server ini, pada branch `hotfix/disable-unrecognized-fallback`) untuk keperluan pairing ulang
sesi WA "direct". CEK & PUTUSKAN ulang status ini (nyalakan lagi atau tetap mati) sebelum mulai coding
v0.13.4.**
...
**Status branch**: `hotfix/disable-unrecognized-fallback` (dari `main`), commit sendiri, TIDAK digabung ke
`v0.26.4-wa-group-dispatch` yang sedang berjalan paralel. Scoped test
`WhatsappIncomingMessageWebhookTest` (19/19 hijau, termasuk kasus fallback generik tetap lolos karena
default config `true`) + Pint bersih. Menunggu merge/tag terpisah (governance BOSS-002 — jangan tinggalkan
lebih dari 1 sesi tanpa merge, per aturan "ATURAN WAJIB — CLOSURE DISIPLIN" di CLAUDE.md).
