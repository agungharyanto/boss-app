...
| v0.23.0 | Network         | OMCI Integration untuk Zero-Touch ONT Provisioning | **Reservasi slot nomor + nama saja — scope detail BELUM ditentukan.** Terpisah TOTAL dari TR-069/GenieACS (sudah dibangun sejak v0.7.x) — OMCI dipakai ONT yang **tidak** support TR-069, butuh integrasi per-vendor OLT (misal SmartOLT REST API untuk ZTE C300 yang sudah dipakai, atau CLI-based untuk OLT vendor lain seperti HSGQ yang mungkin tidak support TR-069 sama sekali). **DHCP Option 43** sedang dipertimbangkan sebagai solusi sementara dengan effort lebih rendah, di luar OMCI penuh. Scope detail (vendor OLT prioritas, DHCP Option 43 vs OMCI penuh) belum ditentukan — decision-gate terpisah saat sprint ini benar-benar dimulai (BOSS-003) — jangan asumsikan detail apa pun dari baris ini. Nomor slot `v0.23.0` — `v0.22.0` ("Manajemen User") didaftarkan di branch `manajemen-user` (belum di-merge saat baris ini ditulis). | Backlog |
