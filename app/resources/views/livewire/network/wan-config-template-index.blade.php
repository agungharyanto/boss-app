<div class="p-6 max-w-6xl mx-auto">
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">{{ __('Matrix Paket x Tipe Modem — pengganti Konfig Remote (fleet-wide). Assignment ke device di Detail Perangkat CPE. Belum sync ke GenieACS.') }}</p>
        </div>
        @if ($canManage)
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <tr wire:key="pkg-{{ $package->id }}-default">
                            ...
                            <td class="px-4 py-2 text-sm text-gray-600 italic">{{ __('Default (Semua Tipe Modem)') }}</td>
                            @if ($defaultTemplate)
                        ...
                        @foreach ($specificTemplates as $template)
                            <tr wire:key="pkg-{{ $package->id }}-template-{{ $template->id }}">
                                <td class="px-4 py-2 text-sm text-gray-800 font-medium">{{ $template->name }}</td>
                                <td class="px-4 py-2 text-sm text-gray-600">{{ $template->modemType->name ?? __('(Tipe Modem dihapus)') }}</td>
                                <td class="px-4 py-2 text-sm">
            ...
            <div class="bg-white rounded-md shadow-lg max-w-lg w-full p-6 space-y-4">
                ...
                <form wire:submit="saveTemplate" class="space-y-3">
                    <div>
                        ...
                        <input type="text" wire:model="name" placeholder="{{ __('Nama bebas, mis. HomeFixed-10Mbps - ZTE Standar') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-400 mt-0.5">{{ __('Hanya Paket yang sudah ada di Profil PPP.') }}</p>
