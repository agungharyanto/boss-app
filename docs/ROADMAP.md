...
| v0.9.11 | Billing & Finance | Payout Komisi — Instan (Titip) & Batch Bulanan (Tanggal 5-7) | Bagian "Payment" dari scope Commission (v0.9.0) yang belum pernah dibangun — v0.9.5 cuma menangani Eligibility (Pending→Eligible), bukan pembayaran aktualnya. Dua mekanisme berbeda: **(1) Titip** — payout instan per transaksi/per grup Referrer, admin bisa langsung tandai dibayar begitu setoran (`deposit_status=sudah_setor`) sudah diterima — tanpa jadwal/batasan tanggal. **(2) Komisi bulanan** (`recurring`/`limited_count`) — payout batch per Referrer, HANYA bisa diproses tanggal 5-7 bulan berjalan (guard di server, bukan cuma UI). | In Progress |
